<!-- fb:creates crates/farmerbob-core/src/backfill.rs -->
# Task: deciding which tasks still need a pipeline, in a shell for-loop

Rust workspace, already builds. Work only inside `crates/farmerbob-core`.
Create `crates/farmerbob-core/src/backfill.rs` and add `pub mod backfill;` to
`crates/farmerbob-core/src/lib.rs`. Do not change any other file.

## Why this exists

`fb-backfill.sh` walks a list of tasks and decides which still need their subjective tier run:
skip if there is no spec, skip if claims already exist, skip if the target cannot be resolved,
otherwise run the pipeline. The list of task names is HARDCODED in the script.

The decision is pure. The hardcoded list is why nobody has run it in weeks, and the skip
reasons are printed and then thrown away, so a task skipped for a bad reason looks identical to
one skipped for a good one.
...
/// Why a task will not be backfilled, or that it will. Exactly these and
/// no others.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum Plan {
...
1. `has_spec == false` is `NoSpec`, whatever else is true. Pin it with claims present and a
   valid target, because the skip order is the contract.
2. `has_spec` with `has_claims` is `AlreadyDone`, even when the target resolves.
3. `has_spec`, no claims, `target: None` is `NoTarget { declared: None }`.
4. `has_spec`, no claims, and a target of the form `crates/<krate>/src/<file>` is
   `Run { krate }` carrying exactly that second path component.
5. A target that does NOT have at least three slash-separated components, or whose second
   component is empty, is `NoTarget` carrying the declared string verbatim. Pin
   `"src/lib.rs"` and `"crates//src/x.rs"`.
6. The precedence is TOTAL and is exactly: `NoSpec` > `AlreadyDone` > `NoTarget` > `Run`. Pin a
   candidate that satisfies two skip conditions at once and assert the higher one.
7. `runnable` returns `(name, krate)` for every `Run` and nothing else, in input order.

## Boundaries, at N and at zero

- `cs` EMPTY: `runnable` is empty.
- A target of exactly `crates/x/src/y.rs`: `Run { krate: "x" }`.
- A target with MORE components, `crates/x/src/a/b.rs`: still `Run { krate: "x" }`. Extra depth
  below `src` is normal and must not become `NoTarget`.
- A target that is the EMPTY string: `NoTarget { declared: Some("") }`, not `None`. A spec that
  declared nothing and a spec that declared an empty string are different faults and the value
  must say which.
- A `name` that is the empty string: degenerate, not invalid. It plans like any other.

## Superset status on every enumerated list

`Plan`'s variants are a CLOSED set of exactly four: `Run`, `NoSpec`, `AlreadyDone`, `NoTarget`.
Adding a fifth is a defect and your tests may not assert on anything outside them.

The target shape recognised is the second form: **at least `crates/<krate>/src/...`; accepting
more shapes is neither required nor penalised**, and your tests may not assert that any other
shape is `NoTarget` beyond the two clause 5 pins.

## Composition of aggregate returns

`runnable` returns one pair per `Run`, in input order, with no deduplication: two candidates of
the same name are two entries, because the caller passed them both and hiding one loses the
duplicate it should be told about.

Say why clauses 1, 2 and 6 must be tested together: clauses 1 and 2 each pass against an
implementation that checks only that one condition, and only clause 6's two-conditions-at-once
candidate distinguishes the order. The order is the whole contract; the conditions are obvious.

## Rules

- No `unwrap()`, `expect()`, `panic!`, `todo!` or `unimplemented!` reachable from input, outside
  `#[cfg(test)]`.
- Add no dependencies and no I/O. The caller lists the directory and reads the specs.
