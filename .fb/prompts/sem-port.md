<!-- fb:creates crates/fb/src/sem_cmd.rs -->
# Task: semaphore is merged and fb-sem.sh counts slots itself

Rust workspace, already builds. Work only inside `crates/fb`.
Create `crates/fb/src/sem_cmd.rs`. Declare it in `main.rs` with one `mod sem_cmd;` line and
change nothing else. `fb` is a binary crate with no `lib.rs`, so `main.rs` is the only place that
line can go.

## Why this exists

`farmerbob_core::semaphore` decides whether a run may take a slot: `grant(&[Slot]) -> Grant`,
and `census(&[Slot]) -> (usize, usize)` for how many are held and how many free. It is merged,
tested, and reachable from nothing. `fb-sem.sh` is 29 lines of shell counting slots itself.

A count that disagrees with the module is worse than no count: admission has already
over-committed a machine once by charging a slot size nobody checked.
...
///
/// One line per slot in the order given, then a summary line carrying the
/// census and what `grant` decided.
...
`Slot` and `Grant` are `farmerbob_core::semaphore`'s. You define neither, and you do not count
slots yourself: `census` and `grant` own both answers.

## Exit codes, pinned

- `0` — `grant` says a slot is available.
- `1` — `grant` says it is not. A caller branches on this with a plain shell `if`.

Nothing else. An empty slot table is not an error.

## Falsifiable clauses

1. Every slot in the input renders one line, in input order, exactly once. Not sorted, not
   filtered, not deduplicated.
2. The summary line carries both numbers `census` returns, and they are `census`'s — assert
   agreement with `semaphore::census` for the same input rather than counting in the test.
3. The exit code agrees with `semaphore::grant` for the same input, for every `Grant` variant.
   Assert the agreement, not a re-derivation: a `match` on slot states in this file is the defect
   this task exists to remove.
4. A table where every slot is held renders every line and exits 1. Pin that the lines are still
   rendered — an operator refused a slot needs to see who holds them.
5. A table with at least one free slot exits 0.
6. Clauses 4 and 5 must be pinned against inputs differing ONLY in one slot's state, so an
   implementation that keys the code off the slot COUNT rather than off `grant` fails.
7. The summary line is non-empty and mentions both census numbers. Assert it contains them;
   do NOT assert wording or layout, neither of which this spec fixes.

## Boundaries, at N and at zero

- ZERO slots: `census` returns whatever it returns for an empty slice and `grant` decides. Render
  the summary line, exit with `grant`'s answer. Assert agreement with the module; do NOT assert a
  code you did not first confirm from `grant(&[])`, and say in your handoff what it turned out to
  be.
- ONE slot, free: exit 0. ONE slot, held: exit 1.
- The census numbers summing to the slot count: pin it. If `census` does not have that property
  for some input, say so in your handoff rather than asserting it.

## Superset status on every enumerated list

The exit codes are EXACTLY 0 and 1.

This module counts nothing and decides nothing. `semaphore::census` and `semaphore::grant` own
both. A `filter`, a `len()` on a filtered slice, or a `match` on `Slot` in this file is scored as
a defect.

## Composition of aggregate returns

`table` returns one line per slot in input order plus exactly one summary line — clause 1 for the
rows, clause 2 for the summary. `run` writes that string and returns a code that reflects only
`grant`'s decision, never the row count.

## Rules

- No `unwrap()`, `expect()`, `panic!`, `todo!` or `unimplemented!` reachable from input, outside
  `#[cfg(test)]`.
- Add no dependencies. No filesystem: the slot table arrives as a value.
...
- RUN `cargo clippy -p fb --all-targets -- -D warnings` BEFORE you finish, including over your
  tests. Two arms tonight shipped lint-dirty test code -- one `needless_range_loop`, six
  `useless use of vec!` -- and both were regressions against a clean baseline.
