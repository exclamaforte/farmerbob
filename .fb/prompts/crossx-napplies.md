...
# Task: "fewer than two candidates" is not a failure, and the acceptance path halts on it
...
`fb trial` ran end to end for the first time and halted at stage four:

```
dispatch      ok
score         ok
differential  ok
crossx        FAILED    "candidates: glm-53-flash / need >=2"
```

A matrix that cannot be built because the field has ONE member is not a failure of anything. It
is the expected state under one-arm-per-task, which is how this project now runs. `run_cmd`
returns `1` for it, and `1` is also what it returns for a usage error, so every caller treats
"cannot apply" and "went wrong" identically — and `fb trial` stops, which makes four of its
eight stages unreachable for a single-arm task. (bead farmerbob-9ef2)

`fb differential` already gets this right with four codes, of which only one is a stage failure.
This gives crossx the same shape.
...
- `1` — a usage error: the bead names no task, or arguments are unusable.
- `3` — the diagonal invariant failed. Scores are deliberately NOT written, because a broken
  transplant must say so rather than emit a confident null. Unchanged.
- `4` — **NOT APPLICABLE: fewer than two candidates.** New. Nothing went wrong and nothing was
  written.

The doc comment on `run_cmd` must state all four, because it is the contract every caller reads.

## Falsifiable clauses

1. A task with ZERO candidates returns 4.
2. A task with ONE candidate returns 4, and still prints the `candidates:` line naming it — an
   operator needs to see which single arm was found.
3. A task with TWO or more candidates is unaffected: whatever it returns today, it returns now.
   Pin at least one existing passing case unchanged.
4. 4 and 1 are DIFFERENT and mean different things. Pin them in one test that produces both,
   because an implementation reusing 1 for the not-applicable case passes clauses 1 and 2 if
   they only assert non-zero.
5. Nothing is written to `<bead>.crossx.json` when 4 is returned. Pin the file's absence, not
   just the code — a stale matrix left behind would be read as current by the next stage.
6. The `run_cmd` doc comment names all four codes and what each means. This is the caller's
   contract and a doc that disagrees with the code is the same defect as a check that cannot
   fail.

## Boundaries, at N and at zero

- ZERO candidates and ONE candidate both return 4: clauses 1 and 2. They are the same answer for
  the same reason, and pinning both stops an implementation special-casing only the one it saw.
- Exactly TWO candidates: the smallest applicable field. Whatever it does today, unchanged.
  Pin that 2 does NOT return 4 — that is the boundary.
- A task whose logs directory does not exist: NOT pinned by this task. Say in your handoff what
  it does and do not change it.

## Superset status on every enumerated list

The exit codes are EXACTLY 0, 1, 3 and 4. `2` stays unused, deliberately — `fb differential`
uses 2 for declared-but-unrunnable and giving crossx a different meaning for the same number
across one binary would be worse than leaving a gap.

This task changes ONLY the not-applicable path and the doc comment. The matrix construction, the
diagonal invariant and the JSON shape are untouched; a change to any of them is out of scope and
will show as one.

## Composition of aggregate returns

`run_cmd` returns ONE code and writes zero or one artefacts. Clause 5 is the pairing a caller
depends on: a non-zero code means no artefact was written this run, so the next stage must not
read one.
...
- No `unwrap()`, `expect()`, `panic!`, `todo!` or `unimplemented!` reachable from input, outside
  `#[cfg(test)]`.
- Add no dependencies.
- Change `crates/fb/src/crossx.rs` and NOTHING else. `fb-crossx.sh` and `fb-pipeline.sh` will
