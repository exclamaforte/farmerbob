<!-- fb:creates crates/farmerbob-core/src/freshness.rs -->
# Task: an artefact must record what it was made from

Rust workspace, already builds. Create `crates/farmerbob-core/src/freshness.rs` and declare
it in `lib.rs`. Change nothing else — the callers are a separate task.

## Why this exists

Four T12 beads, and the backlog already names the shared fix: *content-hash the input, record
it beside the artefact, and re-run when they differ.* Each bead is an artefact outliving the
run that produced it, with nothing noticing.

  farmerbob-03vv  `fb speccheck` reuses a critique written against an OLDER version of the
                  spec. Measured: `speccheck_cmd.rs` records no hash and no sidecar at all,
                  so a critique of a spec that has since changed is indistinguishable from
                  one written this minute.
  farmerbob-dkbu  a spec critique that failed to be written can never be recovered:
                  speccheck refuses once the candidate worktree exists
                  (`speccheck_cmd.rs:197`), so the one state that needs a retry is the one
                  state that forbids it.
  farmerbob-5esi  editing a spec mid-wave shows the critic and the implementer different
                  texts, and nothing records which text either saw.
  farmerbob-29r8  a spent wave file is resurrected by any git tree restore and the autopilot
                  relaunches merged work. Measured: 164 wave files are tracked in git, 163
                  of them under `.fb/queue/dispatched/`, so "already dispatched" is recorded
                  as a git-tracked FILE LOCATION and any checkout can undo it.

`pipeline_cmd` already solves this shape for its stages — `wt_hash`, `needs_run`, and a
`<artefact>.field` sidecar. **This module is that logic, moved where every caller can reach
it, with nothing invented.** The pipeline's behaviour must not change; a later task rewires
it.

## The exact API

```rust
/// A content hash of some input, as recorded beside an artefact.
///
/// Opaque on purpose: callers compare keys, they do not read them. The algorithm is
/// FNV-1a, inlined, matching what `pipeline_cmd::wt_hash` already writes, so sidecars
/// written before this module are still readable by it.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct Key(String);

impl Key {
    /// The key for some bytes.
    pub fn of(input: &str) -> Key;
    /// The key exactly as a sidecar file holds it.
    pub fn as_str(&self) -> &str;
    /// A key read back from a sidecar. `None` for text that is not one.
    pub fn parse(recorded: &str) -> Option<Key>;
}

/// Whether a stage must run.
///
/// This list is CLOSED: a fourth case is a change to this enum.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum Freshness {
    /// The artefact exists and was made from this exact input.
    Fresh,
    /// The artefact exists but was made from different input. Carries the recorded key so a
    /// caller can say what changed.
    Stale { recorded: Key },
    /// No artefact, or no record of what made it. Both mean the same to a caller -- run it --
    /// but they are different facts and a caller may want to say which.
    Absent {
        /// True when the artefact is there and only the record is missing.
        artefact_present: bool,
    },
}

/// Decide whether an artefact is still good for this input.
pub fn assess(artefact_present: bool, recorded: Option<&str>, input: &Key) -> Freshness;
```

## Falsifiable clauses

1. `Key::of` is stable and deterministic: the same input twice gives equal keys, and two
   different inputs give different keys. Pin one pair of each.

2. `Key::of("")` is a real key, not an error. An empty spec is an input like any other, and
   an artefact made from one must be recognised as fresh next time.

3. `assess(true, Some(k), k)` is `Fresh`. `assess(true, Some(other), k)` is
   `Stale { recorded: other }`, carrying the RECORDED key -- not the wanted one -- because
   the caller already knows what it wanted and does not know what it got.

4. `assess(false, _, _)` is `Absent { artefact_present: false }` whatever the record says. A
   record without an artefact records nothing.

5. `assess(true, None, _)` is `Absent { artefact_present: true }`. An artefact whose
   provenance is unknown is NOT fresh: this is the speccheck case, where no sidecar was ever
   written and every stale critique looked current. (03vv)

6. `Key::parse` accepts exactly what `as_str` produces and rejects anything else, returning
   `None` rather than a key that will never match. A malformed sidecar must read as
   `Absent`, which re-runs, and never as a key that happens to differ.

7. Compatibility of SHAPE and ALGORITHM, which is all that can be claimed. `pipeline_cmd::wt_hash`
   hashes a WORKTREE, not a string, so no input produces the same key in both -- the two
   answer different questions. What must match is the machinery, so that sidecars written by
   either are the same kind of thing and one can replace the other later:
...
- A recorded string that is EMPTY is not a key: `Key::parse("")` is `None`, and `assess`
  treats it as `Absent`, not as a key equal to nothing.
- A recorded string with trailing whitespace or a newline is accepted: sidecars are written
  with `write!` and read with `read_to_string`, and a trailing newline is not a difference.
  Pin it.
- Two different inputs may in principle collide. Do NOT add a length or a second hash to
  guard that: FNV-1a over spec text is what the store already holds, and changing it
  invalidates every sidecar. Say so in a doc comment rather than defending against it.

## Composition of the aggregate return

`assess` returns exactly one `Freshness`. `Absent` wins over `Stale`: with no artefact there
is nothing to be stale. The caller's action for `Stale` and `Absent` is the same -- run the
stage -- and they are kept apart so a caller can REPORT which, since "the spec changed" and
"this never ran" send a reader to different places.

## Rules

Pure functions: no file system, no clock, no environment. The caller reads and writes the
sidecars. Do not change `pipeline_cmd`; rewiring it is a separate task and its behaviour must
not move under it. Every clause above gets a test named for what it pins. Keep the workspace
