...
# Task: cross-review cannot happen with one arm, and that is not a failure either
...
`fb crossx` was just given a distinct exit code, `4`, for "fewer than two candidates" — a
not-applicable state, not a fault. Under one-arm-per-task it is the normal state.

`fb critique` has the identical shape and has not been fixed. From `run_cmd`:
...
Two different facts share that `1`, and a third is folded in with them: **fewer than two
candidates**, and **the gate did not pass**. The first is not applicable; the second is a
genuine refusal — there is something to review and the harness declined. Callers cannot tell
them apart, so a one-arm pipeline halts here with `promote` and `prove` never run.
(bead farmerbob-9ef2)
...
- `4` — **NOT APPLICABLE: fewer than two candidates.** Nothing went wrong and nothing was
  written.

The doc comment on `run_cmd` must state all three, because it is the contract every caller
reads. `4` is chosen to match `fb crossx`, so one binary means one thing by one number.

## Falsifiable clauses

1. ZERO candidates returns 4.
2. ONE candidate returns 4.
3. TWO OR MORE candidates with a FAILING gate returns 1, NOT 4. This is the clause that carries
   the task: the two conditions currently share one branch, and separating them is the whole
   change. There is a field to review and the harness is refusing — that is not "not
   applicable".
4. Clauses 2 and 3 must be pinned in tests that differ ONLY in which condition holds. An
   implementation that splits the branch but returns 4 for both passes clauses 1 and 2 and
   fails here.
5. TWO OR MORE candidates with a PASSING gate is unaffected: whatever it returns today, it
   returns now.
6. The `need >= 2` message is printed only when that is actually the reason. When the gate is
   what failed, the message says so instead. Assert the two messages DIFFER; do not assert
   either wording — both are prose.
7. Nothing is written to the critiques directory when 4 is returned. Pin the absence.
8. The `run_cmd` doc comment names all three codes. A doc that disagrees with the code is the
   same defect as a check that cannot fail.

## Boundaries, at N and at zero

- ZERO and ONE candidates both return 4, for the same reason — clauses 1 and 2. Pin both so an
  implementation cannot special-case only the one it saw.
- EXACTLY TWO candidates with a passing gate: the smallest applicable field. Pin that it does
  NOT return 4.
- EXACTLY TWO candidates with a failing gate: returns 1. This is the boundary between clauses 3
  and 5 and both sides must be pinned.
- A candidate list containing the same arm twice: NOT pinned. Say in your handoff what you did
  and do not assert on it.

## Superset status on every enumerated list

The exit codes are EXACTLY 0, 1 and 4. `2` and `3` stay unused here: `fb differential` uses 2
for declared-but-unrunnable and `fb crossx` uses 3 for the diagonal invariant, and one binary
giving one number two meanings across subcommands is worse than leaving gaps.

This task changes ONLY the not-applicable path, the message that accompanies it, and the doc
comment. The derangement, the patch selection and the review dispatch are untouched.

## Composition of aggregate returns

`run_cmd` returns ONE code and writes zero or more critique files. Clause 7 is the pairing a
caller depends on: a 4 means nothing was written this run, so the next stage must not read a
stale directory as current.
...
- No `unwrap()`, `expect()`, `panic!`, `todo!` or `unimplemented!` reachable from input, outside
  `#[cfg(test)]`.
- Add no dependencies.
- Change `crates/fb/src/critique.rs` and NOTHING else. `fb-pipeline.sh` and `fb trial` will
