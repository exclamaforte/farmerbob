<!-- fb:creates crates/farmerbob-core/src/orphan_check.rs -->
# Task: a module nobody declares is a module nobody compiles, and nothing fails

Rust workspace, already builds. Work only inside `crates/farmerbob-core`.
Create `crates/farmerbob-core/src/orphan_check.rs`. Declare it in `lib.rs` with one
`pub mod orphan_check;` line and change nothing else.

## Why this exists

Merging a task whose deliverable is `main.rs` by copying the arm's version wholesale deletes
every `mod` line added to master since that arm branched. It happened: four modules -- four
merged, tested, adjudicated modules -- lost their declarations at once. The files stayed on disk,
unreferenced and uncompiled, and their tests stopped running.

  before   583 tests
  after    541 tests
  restored 608 tests

Nothing failed at any point. `cargo test` was green at every step, because a module nobody
declares is a module nobody compiles, and 42 tests that never run cannot fail. It surfaced only
when a later task happened to import one of them and the build finally complained.
(bead farmerbob-0m34)

The condition is mechanical: every `.rs` file in a crate's `src/` must be declared somewhere in
that crate. This module decides it, so `fb doctor` can assert it on every invocation instead of
someone noticing a test count.
...
///
/// Matches a `mod <stem>;` or `pub mod <stem>;` item, with any leading
/// whitespace and any visibility prefix the language allows before `mod`.
pub fn declares(root: &str, stem: &str) -> bool;
```

## Falsifiable clauses

1. `declares("mod foo;", "foo")` is true; so is `pub mod foo;`, `    mod foo;` and
   `pub(crate) mod foo;`.
2. `declares` is whole-token: `mod foobar;` does NOT declare `foo`, and `mod foo_bar;` does not
   either. Pin both — a `contains` implementation passes clause 1 and fails only here, and this
   crate has `run_timing` beside `run_state_view` and `compare_cmd` beside `compare_gather`.
3. A `mod foo;` inside a ``` fenced block in a doc comment does NOT count. Pin it; specs and
   module docs in this repository quote declarations routinely.
4. `mod foo { ... }` — an INLINE module with a body — does not declare the FILE `foo.rs`.
   Assert it is not treated as a declaration of the file.
5. A stem declared in `lib.rs` is not an orphan. A stem declared in `main.rs` is not an orphan.
   A stem declared in NEITHER is an orphan. Pin all three.
6. When a crate has both roots, a declaration in EITHER suffices. Pin a stem declared only in
   `main.rs` while `lib_rs` is `Some` and does not declare it.
7. The stems `lib` and `main` are never orphans, whether or not anything declares them. They are
   the roots. Pin it — the obvious implementation reports both on every crate and a check that
   fires on everyone is read by no one.
8. `orphans_in` returns orphans in the order `stems` lists them, none dropped, none
   deduplicated.
9. `orphans` concatenates per crate, crates in the order given, each crate's orphans in its own
   stem order.
10. A crate with `lib_rs: None` and `main_rs: None` and a non-empty `stems`: every stem except
    `lib` and `main` is an orphan. There is no root to declare anything.

## Boundaries, at N and at zero

- ZERO crates: `orphans(&[])` is empty.
- A crate with ZERO stems: no orphans, whatever the roots say.
- A crate whose only stem is `lib`: no orphans, per clause 7.
- A stem appearing TWICE in `stems`: degenerate, not invalid. Clause 8 says no deduplication, so
  an undeclared stem listed twice yields two orphans. Pin it.
- An empty stem string: NOT pinned. Say in your handoff what you did and do not assert on it.
- A root that is an empty string: declares nothing. Every stem is an orphan.

## Superset status on every enumerated list

The visibility prefixes `declares` must accept are AT LEAST: none, `pub`, `pub(crate)`,
`pub(super)`, `pub(in path)`. Accepting more is neither required nor penalised, and **your tests
may not assert on cases outside this list**.

The two root names are EXACTLY `lib` and `main`.

This module reads no filesystem. The caller lists the files and reads the roots; this decides.

## Composition of aggregate returns

`orphans` is the concatenation of `orphans_in` over the input, in order — clause 9 — so the two
cannot disagree. Pin that `orphans(&[c])` equals `orphans_in(&c)` for the same crate.

Each `Orphan` carries the crate name it came from, so a concatenated list stays attributable.

## Rules

- No `unwrap()`, `expect()`, `panic!`, `todo!` or `unimplemented!` reachable from input, outside
  `#[cfg(test)]`.
- Add no dependencies, and no `regex`. Hand-written scanning is expected.
