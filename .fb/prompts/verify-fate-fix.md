<!-- fb:modifies crates/fb/src/verify_gather.rs -->
# Task: fb verify says "the suite ran no tests" about a suite that passed six

Rust workspace, already builds. Work only inside `crates/fb`.
Modify `crates/fb/src/verify_gather.rs`. Do not change any other file.

## Why this exists

```
$ fb verify sem-port
codex-luna: failed -- the suite ran no tests
exit 1
```

`sem-port`'s arm wrote six tests and they passed. The scorer recorded
`verdict: "PASS"`, `build: "pass"`, `test: "pass"`, `tests_run: 547`.

The mapping is wrong, and it is wrong in the specification this module was built from, not in
any of the code beneath it. `verify_gather` sets `Raw::test_log` from the scorer's `err` field.
`err` is the ERROR field and it is EMPTY for a passing run. `verify_cmd` hands that empty string
to `build_verdict::read(built, test_log)`, which scans for `test result: ok. N passed`, finds
none, and correctly returns `NoTests` — which `verify_plan` correctly turns into a failure.

Every module did what it was told. `score.json` contains no test log, so reconstructing a
verdict by re-parsing one cannot work. It contains the ALREADY-PARSED result. (bead
farmerbob-35x1)

## Exact API

`rows_for` and `run` keep their signatures. `Raw::test_log` stops being derived from `err` and
instead carries a SYNTHETIC log that `build_verdict::read` will parse into the verdict the
scorer already recorded:

- `build: "pass"`, `test: "pass"`, `tests_run: N` — a log that `read` parses as `Passed { passed: N }`.
- `build: "pass"`, `test: "FAIL"` — a log that `read` parses as a test failure.
- `build: "FAIL"` — `Raw::built` is already `Some(false)`, so `read` returns `BuildFailed`
  before looking at the log; the log's content does not matter and you must not depend on it.
- anything else — `Raw::built` is `None` and the fate is decided without the log.

**Do not hand-write the log format from memory.** Read `build_verdict`'s parser and produce text
it actually accepts. If you cannot produce a string that `read` parses as `Passed { passed: N }`
for some N, say so in your handoff and name the obstacle rather than shipping something that
almost works.

## Falsifiable clauses

1. A record with `build: "pass"`, `test: "pass"`, `tests_run: 6` produces a `Raw` for which
   `build_verdict::read(built.unwrap(), &test_log)` returns `Passed { passed: 6 }`. Assert
   through `read`, not by inspecting the string — the string's shape is not the contract, the
   parse result is.
2. `tests_run: 0` with both passing: `read` returns whatever it returns for zero passing tests.
   Assert agreement with `read` rather than a value you did not confirm, and say in your handoff
   what that turned out to be.
3. `test: "FAIL"` produces a `Raw` whose `read` result is a test failure, NOT `NoTests` and NOT
   `Passed`. Pin all three outcomes as distinct.
4. `build: "FAIL"` yields `built: Some(false)`, and `read` returns `BuildFailed` regardless of
   the log. Pin that the log content does not change this.
5. An absent or unrecognised `build` still yields `built: None`, unchanged from today.
6. **End to end: a record equivalent to sem-port's — `PASS`/`pass`/`pass`/547 — must produce a
   fate of `Measure`, not `Failed`.** Assert through `verify_cmd`'s public surface, because the
   whole defect is that every layer was individually right and the composition was wrong.
7. `deliverable` and `timed_out` keep their current mapping exactly: `NO-OP` and
   `INDETERMINATE` respectively. Pin one of each so this change does not disturb them.

## Boundaries, at N and at zero

- `tests_run` absent with both fields passing: NOT pinned. Say in your handoff what you produced
  and do not assert on it.
- `tests_run` present but not a number: same instruction.
- A record with `test: "pass"` and `build: "FAIL"`: contradictory, and the scorer should not
  emit it. `built: Some(false)` wins per clause 4. Pin it — a contradiction must resolve
  deterministically rather than depending on field order.

## Superset status on every enumerated list

The fields read stay EXACTLY `source`, `build`, `test`, `verdict`, `err`, plus `tests_run` which
this task adds. That is six, and adding a seventh means this and `fb brief` can disagree about
what a run was.

`err` is no longer used to build `test_log`. Say in your handoff whether you kept reading it for
anything else; if nothing needs it, say that too.

## Composition of aggregate returns

Clause 6 is the composition clause and it is the point of the task: `rows_for` produces values,
`verify_cmd` interprets them, and the two were individually correct while their composition was
not. A test that stops at `rows_for` cannot see this defect, which is why one is required to
reach the fate.
...
- No `unwrap()`, `expect()`, `panic!`, `todo!` or `unimplemented!` reachable from input, outside
  `#[cfg(test)]`.
- Add no dependencies.
- Change `crates/fb/src/verify_gather.rs` and NOTHING else. If you conclude the right fix is to
  change `verify_cmd::Raw` to carry a `BuildVerdict` instead of a log, SAY SO IN YOUR HANDOFF
  with your reasoning — that is a better design and it is a different task.
