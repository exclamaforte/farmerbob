<!-- fb:creates crates/farmerbob-core/src/stage_exit.rs -->
# Task: three near-identical case blocks in shell, and the rule they encode exists nowhere

Rust workspace, already builds. Work only inside `crates/farmerbob-core`.
Create `crates/farmerbob-core/src/stage_exit.rs`. Declare it in `lib.rs` with one
`pub mod stage_exit;` line and change nothing else.

## Why this exists

Four pipeline stages were given a NOT-APPLICABLE exit code this week, because each conflated
"cannot apply" with "failed" and halted a one-arm pipeline:

- `fb differential` — 0 arms agree, 1 an arm diverges, 2 declared-but-unrunnable, 3 no oracle.
  Only 2 is a failure.
- `fb crossx` — 0 matrix built, 4 fewer than two candidates, 3 diagonal invariant failed,
  1 usage error.
- `fb critique` — 0 ran, 4 fewer than two candidates, 1 gate did not pass.
- `fb promote` / `fb-prove.sh` — 4 nothing to promote, nothing to prove.

`fb-pipeline.sh` now carries FOUR hand-written `case` blocks mapping those codes to an action,
and `crates/fb/src/trial.rs` carries two more in Rust. They are near-identical, none is tested,
and the rule they encode — which codes mean carry on, which mean stop — exists in no single
place. The shell grew by roughly two hundred lines this week acquiring them.
...
pub enum StageExit {
    ...
    Done,
    /// It could not apply, and nothing went wrong. Continue WITHOUT recording a
    /// signature: nothing was produced, so there is nothing to cache.
    NotApplicable,
    /// It failed. Stop. Carries the raw code, so a caller can report it.
    Failed(i32),
}

/// One stage's exit-code contract: the codes that mean NotApplicable.
///
/// `Done` is ALWAYS exactly 0 and is not listed. Every code that is neither 0
/// nor in `not_applicable` is `Failed`.
pub struct Contract {
    /// The stage's name, for reporting.
    pub stage: String,
    /// Codes meaning "could not apply". May be empty: most stages have none.
...
1. Code 0 is `Done`, whatever the contract lists — even if 0 appears in `not_applicable`.
   Pin that degenerate contract: 0 always means the stage did its work.
2. A code listed in `not_applicable` is `NotApplicable`.
3. Any other non-zero code is `Failed`, carrying that code unchanged. Pin the payload.
4. `proceeds` is true for `Done` and for `NotApplicable`, false for `Failed`. Pin all three.
5. `caches` is true for `Done` ONLY. `NotApplicable` does NOT cache. Pin clauses 4 and 5 as a
   PAIR against the same `NotApplicable` value: it continues but must not record a signature,
   because nothing was produced and a cached signature would make the next run skip a stage that
   never ran. An implementation that returns the same answer for both passes clause 4 alone.
6. An EMPTY `not_applicable` makes every non-zero code `Failed`. That is the common case — most
   stages have no not-applicable state — and it must not require a special contract.
7. A NEGATIVE code, which is what a signal-killed process reports through some launchers, is
   `Failed` carrying the negative value verbatim, never normalised. Pin `-9`.
8. The contract's `stage` name is carried nowhere in the return. It is for the caller's message
   only; assert that two contracts differing only in `stage` classify identically.

## Boundaries, at N and at zero

- `not_applicable: []`: clause 6.
- `not_applicable` containing 0: clause 1 — `Done` wins.
- `not_applicable` containing the SAME code twice: degenerate, not invalid. Still
  `NotApplicable`. Pin it.
- `i32::MIN` and `i32::MAX` as codes: `Failed`, carrying them. Nothing overflows.
- An empty `stage` name: not pinned. Say in your handoff what you did and do not assert on it.

## Superset status on every enumerated list

`StageExit` has EXACTLY three variants. Closed set.

This module does NOT know which codes any particular stage uses. The four real contracts —
differential, crossx, critique, prove — belong to the CALLER, which reads them from wherever it
configures stages. A `const CROSSX_CONTRACT` or a `match` on a stage name in this file is the
defect this task exists to remove: a fifth stage must not require editing it.

`farmerbob_core::stage_key::Produced` already classifies a stage's outcome from an exit code AND
an artefact's presence. This is a different question: it is about the code alone, and about
whether a code means not-applicable. If you believe the two are the same concept, say so in your
handoff and name the clause `Produced` cannot express, rather than merging them yourself.
**Where this spec's Exact API and an existing type conflict, the EXACT API WINS.**

## Composition of aggregate returns

`classify` returns exactly one `StageExit`. `proceeds` and `caches` read it and answer different
questions; clause 5 is the statement that they are not the same question, which is the whole
reason both exist.
...
- No `unwrap()`, `expect()`, `panic!`, `todo!` or `unimplemented!` reachable from input, outside
  `#[cfg(test)]`.
- Add no dependencies. No filesystem and no process spawning: the code arrives as an `i32`.
