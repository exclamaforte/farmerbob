<!-- fb:creates crates/farmerbob-core/src/spec_fate.rs -->
# Task: a queued spec that can no longer run must say so, instead of being listed as work

Rust workspace, already builds. Work only inside `crates/farmerbob-core`.
Create `crates/farmerbob-core/src/spec_fate.rs`. Declare it with one `pub mod spec_fate;` line
in `crates/farmerbob-core/src/lib.rs` and change nothing else.

## Why this exists

Merging a winner invalidates the specs still sitting in the queue. `bandit-route` was written
against a base where `crates/farmerbob-core/src/router.rs` did not exist. `router.rs` was merged
months of waves ago. The spec still declares `fb:creates router.rs`, and it is still listed as
available work — so dispatching it spends an arm on a task whose only correct outcome is to do
nothing. Six of the twelve unspent specs in the queue are in this state right now.

The check exists, in shell, inside `fb-status.sh`, as a two-line `if` that asks whether the
declared path exists on disk. It is right, and it is in the wrong place: nothing else can call
it, the dispatcher does not consult it, and it decides on `[ -f "$tgt" ]` alone.

This is the same failure this project keeps finding: a spec that cannot run is indistinguishable
from a spec that has not been run yet. (bead farmerbob-m71)
...
pub enum Fate {
    ...
    Undecidable(crate::target_decl::NoDeclaration),
}
...
`survey` takes `(name, spec_text)` pairs and returns `(name, fate)` pairs.

The declaration comes from `crate::target_decl::declared`. Do not re-parse the marker; that
parser already handles fenced code blocks, ambiguity and refusal, and a second one here would
disagree with it on exactly the inputs that matter.

## Falsifiable clauses

1. A spec declaring `fb:creates a/b.rs` against a base where `a/b.rs` exists is
   `Stale { path: "a/b.rs", .. }`.
2. The SAME spec against a base where `a/b.rs` does not exist is `Runnable`. Clauses 1 and 2
   differ in one bit and must be pinned as a pair.
3. A spec declaring `fb:modifies a/b.rs` inverts it: `Runnable` when `a/b.rs` EXISTS, and
   `Stale` when it does not. A modifies-task against a missing file is just as dead as a
   creates-task against a present one, and the shell check today gets this case wrong — it only
   ever tests `creates`. Pin both halves.
4. `Stale::grounds` is prose. It is non-empty, and it mentions the path. Its exact wording is
   not pinned and your tests may not assert on the sentence.
5. A spec with no `fb:` marker is `Undecidable(NoDeclaration::Absent)` — not `Runnable`. A spec
   nobody can place is not a spec anybody should dispatch.
6. A spec with two markers is `Undecidable(NoDeclaration::Ambiguous(..))`, and the base is not
   consulted at all. Pin that a `Base` whose `exists` panics is never called for such a spec.
7. `fate` is a pure function of the spec text and the base's answers. Calling it twice with the
   same inputs gives the same answer.

## Boundaries, at N and at zero

- `survey(&[], base)` returns an empty `Vec`. Not an error.
- `survey` over one spec returns one entry, and it equals `fate` on that spec. Pin the
  agreement rather than restating the logic.
- Two specs with the SAME name are two entries. `survey` returns a `Vec`, not a map, and it
  must not deduplicate: the queue is a list of files and two files can declare the same target.
- A declared path that is the empty string: decide it, and say in your handoff which way and
  why. It is not pinned here.

## Superset status on every enumerated list

`Fate` has exactly these three variants and no others. A fourth would be a defect.

`Base` has exactly one method. Do not add filesystem access to this module — the reason the
shell check is untestable is that it reaches for the disk, and a module that does the same is
no improvement.

## Composition of aggregate returns

`survey` returns exactly one entry per input, in input order, each carrying the name it was
given. Nothing is filtered — a caller wanting only the stale ones filters them itself, and a
caller wanting a count of what is genuinely available needs the total to divide by.
...
- Create `crates/farmerbob-core/src/spec_fate.rs` plus the one `pub mod` line, and nothing else.
