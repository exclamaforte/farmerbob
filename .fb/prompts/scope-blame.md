<!-- fb:modifies crates/farmerbob-core/src/scope.rs -->
# Task: a departure the base caused must not weigh the same as one the agent chose

Rust workspace, already builds. Work only inside `crates/farmerbob-core`.
Modify `crates/farmerbob-core/src/scope.rs`. Do not change any other file.

## Why this exists

`Scope::departures` is a flat list and `is_clean` is a boolean over it, so every departure
counts the same. That threw away good work today.

score-delta/codex-luna wrote a careful 272-line implementation of its declared target and
scored `OUT-OF-SCOPE` on seventeen departures. All seventeen were `cargo fmt` — line-wrapped
`assert!` calls in files the task never mentioned, semantically null, and provably so:

```text
for f in <the 17>; do git diff --ignore-all-space --ignore-blank-lines <base> <wt> -- $f; done
-> empty for all 17
```

The scope gate exists to stop an arm reaching outside its task and improving the workspace so
it looks better. A formatting-only diff cannot do that. The run was discarded anyway, because
seventeen was the only number the type could express.

`farmerbob_core::fmt_gate` already decides the attribution half — `Blame::Agent` versus
`Blame::BaseWasUnformatted`, keyed on which files the formatter would have rewritten on the
base. What is missing is scope's own ability to carry the distinction at all.

## Exact API

`Departure` gains a kind. The two existing variants keep their names, their fields and their
meanings:
...
pub enum Departure {
    Foreign { path: String, kind: Kind },
    Deleted { path: String, kind: Kind },
}
```

`Change` gains the one input needed to decide it:
...
`Scope` gains one method's worth of arithmetic, as free functions beside `is_clean`:

```rust
/// Departures the agent is answerable for: those of kind `Semantic`.
///
/// This is the figure a verdict keys on. `scope.departures.len()` is the
/// figure to REPORT, and they are different numbers on purpose.
pub fn semantic_departures(scope: &Scope) -> usize;

/// Whether the run departed in a way that could affect the measurement.
///
/// False for a run whose every departure is `FormattingOnly`, true as soon
/// as one is `Semantic`.
pub fn is_semantically_clean(scope: &Scope) -> bool;
```

`is_clean` KEEPS its current meaning: true only when there are no departures at all, of any
kind. It answers "did this run stay exactly inside its deliverable", which is still a question
worth asking, and other code reads it.

A `Deleted` departure is **always `Semantic`**, whatever `formatting_only` says. Deleting a file
is not a formatting change, and a caller that sets the flag on a deletion is confused rather
than informative.

## Falsifiable clauses

1. A change to a foreign path with `formatting_only: false` is
   `Foreign { kind: Kind::Semantic, .. }`; with `formatting_only: true` it is
   `Foreign { kind: Kind::FormattingOnly, .. }`. Pin both over one input containing both.
2. **The headline.** A scope whose every departure is `FormattingOnly` has
   `semantic_departures == 0` while `departures.len()` is non-zero, and `is_semantically_clean`
   is true while `is_clean` is FALSE. Pin all four on the one value. A verdict keying on the
   wrong one is the defect this task exists to remove.
3. One `Semantic` departure among any number of `FormattingOnly` ones makes
   `is_semantically_clean` false. Pin it with the semantic one LAST in the input, so an
   implementation that stops at the first departure is caught.
4. A `Deleted` change is `Kind::Semantic` even when `formatting_only` is true. Pin it.
5. `is_clean` is unchanged for every existing case: false whenever `departures` is non-empty,
   whatever the kinds. Pin a formatting-only field as the case that separates it from
   `is_semantically_clean`.
6. `target_changed` and `allowed` are untouched by this change. Pin one existing case of each.

## Boundaries, at N and at zero

- No departures at all: `semantic_departures == 0`, `is_semantically_clean` true, `is_clean`
  true. All three agree, and that agreement is what makes clause 2's disagreement meaningful.
- Every departure formatting-only, exactly one of them: `semantic_departures == 0`. The
  boundary is at one, not at some threshold.
- A change whose path is the declared target: not a departure at all, and `formatting_only` has
  no bearing on that. Pin it.
- An allowed path with `formatting_only: true`: still allowed, still not a departure.

## Superset status on every enumerated list

`Kind` has EXACTLY two variants. A third — "unknown", "not measured" — is a defect here: where
the caller could not measure whether a diff was whitespace-only, that is the caller's
`Measurement` to carry, and `Change::formatting_only` is a plain `bool` on purpose. Do not
widen it.

`Departure` keeps exactly its two existing variants, each gaining exactly one field.

## Composition of aggregate returns

`Scope::departures` still holds one entry per departing path, sorted ascending by path, no
duplicates — unchanged. `semantic_departures` counts entries of that same list and never
filters it: a caller reporting "17 departures, none of them semantic" needs both numbers, and
clause 2 is the statement that it can have them.

## Rules

- No `unwrap()`, `expect()`, `panic!`, `todo!` or `unimplemented!` reachable from input, outside
  `#[cfg(test)]`.
- Add no dependencies. No I/O: this module never runs git and never runs a formatter.
- Derive `Debug, Clone, Copy, PartialEq, Eq` on `Kind`; keep the existing derives on
  `Departure`, `Scope` and `Change`.
- Every construction of `Change` inside this file's own tests must set the new field. Callers in
  OTHER crates will stop compiling, and that breakage is EXPECTED: leave it, and say in your
  handoff which callers you saw break.
- The base you are given is rustfmt-clean; `cargo fmt` will touch your file and nothing else.
- Change `crates/farmerbob-core/src/scope.rs` and nothing else, plus `.fb/handoff.md`.
