<!-- fb:modifies crates/fb/src/eligible.rs -->
# Task: the arm-refusal reader has one caller and the registry has five statuses

Rust workspace, already builds. Work only inside `crates/fb`.
Modify `crates/fb/src/eligible.rs`. Do not change any other file.

## Why this exists

`sources.toml` now carries twelve `oc-*` arms alone, in three states: `verified`, `untested`
and `disabled`, plus `parked_until` on some entries. `fb eligible` decides dispatchability and
prints a reason when it refuses, and that reason is the only thing telling an operator why a
wave came back short.

Two of tonight's registrations are `disabled` carrying a measured HTTP 410 end-of-life notice;
seven are `untested` and have never run. An operator reading a refusal needs to know WHICH of
those it hit, because they mean opposite things: one will never work again, the other has simply
not been tried.
...
pub fn classify(status: Option<&str>, parked_until: Option<&str>, reason: Option<&str>) -> Eligibility;
...
1. `status: Some("verified")` with no park: `Verified`, and `dispatchable` is true.
2. `status: Some("untested")` with no park: `Untested`, and `dispatchable` is TRUE. An arm that
   has not been tried is not an arm that failed. Pin the truth value — refusing untested arms
   would make a new registration unreachable, and this project has nine of them.
3. `status: Some("disabled")`: `Disabled`, carrying the reason unchanged, `dispatchable` false.
4. `parked_until: Some(t)` with `status: Some("verified")`: `Parked(t)`, `dispatchable` false.
   A park OUTRANKS a verified status — pin it, because the obvious implementation checks status
   first and returns `Verified`.
5. `status: None`: `Unregistered`. Pin that this differs from `Disabled` — one is a typo in a
   wave file and should stop the wave, the other is a deliberate policy and should not.
6. An unrecognised status string — `Some("retired")`: NOT dispatchable, and NOT `Verified`.
   Which variant it maps to is yours; say so in your handoff and pin only that it is refused.
   Treating an unknown status as dispatchable is the dangerous direction.
...
- `parked_until: Some("")`: NOT pinned. Say what you did and do not assert on it.
- Both `disabled` and `parked_until` set: `Disabled` wins, because it is permanent and the park
  will expire into nothing. Pin it.

## Superset status on every enumerated list

`Eligibility` has EXACTLY five variants. The statuses recognised are EXACTLY `verified`,
`untested` and `disabled`; anything else is refused per clause 6.

This module does NOT read `sources.toml`, does not parse TOML, and does not compare a park time
against a clock. Every value arrives as an `Option<&str>` and the caller owns both the file and
the clock. A `chrono` call or a file read here is a defect.

## Composition of aggregate returns

`classify` returns exactly one `Eligibility` and `dispatchable` reads it — clause 4 is the
statement that the two inputs are not independent, and that the refusing one wins.

## Rules

- No `unwrap()`, `expect()`, `panic!`, `todo!` or `unimplemented!` reachable from input, outside
  `#[cfg(test)]`.
- Add no dependencies. No filesystem, no clock.
- Derive `Debug, Clone, PartialEq, Eq` on the types this task DEFINES.
- Change `crates/fb/src/eligible.rs` and NOTHING else. If existing callers in that file need
  adjusting to compile, that is in scope; if a caller in ANOTHER file breaks, leave it broken
