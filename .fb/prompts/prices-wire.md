...
# Task: fb prices still reports a clean bill from a command that never asked
...
`fb prices` prints `nothing pending` and exits 0 without reading a catalogue. That is the same
sentence and the same exit code it prints when every registered price genuinely AGREES, and 0 is
what a caller treats as no-action-needed. (bead farmerbob-qvrs)

`prices_gather` was merged to close this and has no caller: it reads `sources.toml` and a
catalogue into `price_update::pending`, returning 4 when a source cannot be read, 0 for a real
all-clear and 1 for pending changes. The subcommand still passes an empty slice.

## Exact API

The `Prices` subcommand gains two arguments:
...
Its arm calls `prices_gather::run` with those paths, writing to `std::io::stdout()`, and exits
with the returned code via `std::process::exit`.

`--catalogue` is REQUIRED and has no default. That is the point of the task: a command that
cannot name its catalogue must not run and report an all-clear.

## Falsifiable clauses

1. `fb prices --catalogue <file>` on a readable registry and catalogue exits with
   `prices_gather::run`'s code, unchanged, and writes what it writes.
2. `fb prices` with NO `--catalogue` is clap's own missing-argument error, exit 2, and prints
   NOTHING on stdout. Pin the empty stdout — this is the case that used to say
   `nothing pending` and exit 0.
3. A `--catalogue` path that does not exist exits 4, not 0. Pin that 4 and 0 differ here: an
   unreadable catalogue and a genuine all-clear are the two states this whole chain exists to
   separate.
4. `--sources` defaults to the repository's `sources.toml` when omitted. Pin that omitting it
   does not error.
5. A `--sources` path that does not exist exits 4, naming that path rather than the catalogue.
   Assert the output mentions the sources path; do not assert the wording.
...
- Neither file exists: exit 4. Which one is named first is NOT pinned — say in your handoff
  which you report and do not assert on it.
- An empty catalogue file that is valid JSON `[]`: whatever `prices_gather` returns; assert
  agreement with it rather than a value you did not confirm.
- `--catalogue` given a directory: exit 4.

## Superset status on every enumerated list

The `Prices` subcommand takes EXACTLY these two arguments. The exit codes are EXACTLY 0, 1, 2
and 4, where 2 is clap's.

This task adds NO logic. Reading, comparing and rendering all belong to `prices_gather`,
`price_update` and `prices_cmd`. A default catalogue path baked into `main.rs`, or a fallback
that runs with no catalogue, is the defect this task exists to remove.

## Composition of aggregate returns

The subcommand writes what `prices_gather::run` writes and exits with what it returns — clause
1. Clause 2 is the one a caller depends on: with no catalogue named, nothing is written and the
process fails, so a cron cannot mistake silence for agreement.
...
- Add no dependencies.
- Change `crates/fb/src/main.rs` and NOTHING else. Note that `prices_cmd` may then have an
  unnecessary `#![allow(dead_code)]`; name it in your handoff rather than editing that file.
