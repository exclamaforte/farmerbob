<!-- fb:creates crates/fb/src/verify_gather.rs -->
# Task: verify_cmd renders a fate nobody can feed it

Rust workspace, already builds. Work only inside `crates/fb`.
Create `crates/fb/src/verify_gather.rs`. Declare it in `main.rs` with one `mod verify_gather;`
line and change nothing else. `fb` is a binary crate with no `lib.rs`.

## Why this exists

`verify_cmd` renders what should happen to each candidate — measure it, treat it as a no-op,
blame the arm, or blame the machine — and takes `&[Raw]`. Nothing builds a `Raw`. It is one of
three modules in this crate that remain reachable from nothing, and `fb-verify.sh` is still 107
lines of shell making the decision itself.

`compare_gather` is the pattern: read the task's `score.json`, build the values, hand them to
the renderer. This is that layer for verify.

## The score file's shape, pinned

Each element is an object. The fields this module reads are EXACTLY these five:

- `source` — the arm name, a string.
- `build` — a string. Observed values across 483 records: `"pass"` and `"FAIL"`, and nothing
  else. Treat any other value, or an absent field, as UNKNOWN rather than as failure.
- `test` — a string, same two observed values, same rule.
- `verdict` — a string. Observed: `PASS`, `NO-OP`, `INDETERMINATE`, `TESTS-FAIL`, `NO-COMPILE`,
  `NO-TESTS`, `OUT-OF-SCOPE`.
- `err` — a string, possibly empty.

That list of verdicts is "at least these; accepting more is neither required nor penalised" —
an eighth verdict may appear and your tests may not assert on cases outside the list.
...
`Raw` is `crate::verify_cmd::Raw`. You do not define it, you do not render anything, and you do
not decide any fate: `verify_cmd` and `farmerbob_core::verify_plan` own that.

## Mapping, pinned

- `Raw::arm` is `source`.
- `Raw::deliverable` is true when `verdict` is anything other than `NO-OP`. A no-op is the
  recorded state for an arm that wrote nothing; every other verdict implies something was
  written.
- `Raw::built` is `Some(true)` for `build == "pass"`, `Some(false)` for `build == "FAIL"`, and
  `None` for anything else including an absent field. `None` means NOT ATTEMPTED, which is not
  the same as failed.
- `Raw::test_log` is `err`, or the empty string when absent. This module does not invent a log.
- `Raw::timed_out` is true when `verdict` is `INDETERMINATE`. That is the verdict the scorer
  records when a run was cut short, and it is the one thing `verify_plan` must not blame on the
  arm.

## Falsifiable clauses

1. A record with `verdict: "PASS"`, `build: "pass"`, `test: "pass"` yields `deliverable: true`,
   `built: Some(true)`, `timed_out: false`.
2. `verdict: "NO-OP"` yields `deliverable: false`. Pin it against clause 1 — the two differ only
   in that field and it decides whether the arm is blamed for writing nothing.
3. `verdict: "INDETERMINATE"` yields `timed_out: true`. Pin that this is the ONLY verdict that
   does, using `TESTS-FAIL` as the contrast: one is the machine's fault and one is the arm's.
4. `build` absent, or any string other than the two observed, yields `built: None`, NOT
   `Some(false)`. Pin absent and unrecognised separately. An unattempted build reported as a
   failed one blames an arm for something that never ran.
5. `err` absent yields an empty `test_log`; `err` present yields it unchanged.
6. Rows come back in the array's order, one per element, none dropped — including a record whose
   every field is missing except `source`.
7. A path that does not exist yields `Unreadable`; a top level that is not an array yields
   `Malformed`. Pin that the two differ.
8. `run` returns `verify_cmd::run`'s code for a readable file, and a DIFFERENT non-zero code for
   each `GatherError`. State which you chose in your handoff; the spec pins only that the three
   are distinct.

## Boundaries, at N and at zero

- An EMPTY array: `Ok(vec![])`, not an error. A task with no candidates is a readable file, and
  `verify_cmd::run` already decides what an empty field does.
- A record missing `source`: NOT pinned. Say in your handoff whether you dropped it, named it
  empty, or errored, and do not assert on it.
- A record that is not an object inside a valid array: `Malformed`.
- A file containing only whitespace: `Malformed`, not `Unreadable`. It was read.

## Superset status on every enumerated list

The five fields above are EXACTLY what this module reads. The scorer writes twenty-one; reading
a sixth means this and `fb brief` can disagree about what a run was.

`GatherError` has EXACTLY two variants.

## Composition of aggregate returns

`rows_for` returns one `Raw` per array element in array order — clause 6 — and each row's four
derived fields are independent: `built: None` says nothing about `deliverable`. Pin a row with
`deliverable: true` and `built: None`, which is a real combination for an INDETERMINATE run.

`run` composes `rows_for` with `verify_cmd::run`: on success its code is that function's,
unchanged.
...
- Tests write their own fixture files under a temporary path. Do NOT read this repository's
...
- Do NOT wire a subcommand; that is a separate task. Say so in your handoff, and NAME any
  `#![allow(dead_code)]` you add so a follow-up removes it — three modules in this crate are
  currently unreachable behind exactly that attribute.
