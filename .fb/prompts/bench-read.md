<!-- fb:creates crates/farmerbob-core/src/bench_read.rs -->
# Task: nothing turns a benchmark's output into the values the scorer needs

Rust workspace, already builds. Work only inside `crates/farmerbob-core`.
Create `crates/farmerbob-core/src/bench_read.rs`. Declare it in `lib.rs` with one
`pub mod bench_read;` line and change nothing else.

## Why this exists

`task_contract::score(manifest, verify, samples, baseline_ms, max_spread)` decides a benchmark
task's verdict, and `gate_kind` can now turn that verdict into a gate decision. Neither has an
input. Nothing in this repository produces a `VerifyOutput` or a `BenchOutput` from anything a
benchmark actually prints.

The contract already names the producers — `BenchOutput` is documented as "the output printed by
`bench.sh`" and `VerifyOutput` as "the output printed by `verify.sh`" — and those scripts are
run per task, once per trial. This module reads what they print. (bead farmerbob-p172)

Getting this wrong in the obvious way is the failure this project keeps hitting: a benchmark
that printed nothing, or printed something unparseable, must not read as a fast kernel or a
correct one.
...
pub enum ReadError {
    ...
    Unparseable(String),
    /// It printed a number that cannot be a measurement: negative, NaN or
    /// infinite. Carries the offending text.
...
///
/// Returns the successful samples in input order, and every failure with the
/// index of the trial it came from.
...
`VerifyOutput`, `BenchOutput` and `TaskManifest` are `farmerbob_core::task_contract`'s. You
define none of them.

## The wire format, pinned

Both scripts print ONE line of JSON on stdout. `verify.sh` prints an object with `correct`
(boolean) and `detail` (string). `bench.sh` prints an object with `ms` (number) and, optionally,
`metrics` (any JSON value; absent means `serde_json::Value::Null`).

Anything else on stdout — log lines before or after — is IGNORED: the LAST line that parses as
a JSON object with the required field is the result. This is pinned because a benchmark that
prints progress to stdout is normal and must not be unreadable for it.

## Falsifiable clauses

1. `verify` on `{"correct":true,"detail":"all close"}` with `exited_zero: true` yields that
   `VerifyOutput`.
2. `verify` on `{"correct":false,"detail":"mismatch at 3"}` yields `correct: false`. This is a
   SUCCESSFUL READ of a failed verification — `Ok`, not `Err`. Pin it: an incorrect kernel is a
   measurement, and confusing it with an unreadable run is the defect this module exists to
   avoid.
3. `exited_zero: false` yields `Err(Failed)` carrying the stdout, whatever the stdout says —
   even if it contains valid JSON. A process that failed did not produce a measurement.
4. Clauses 2 and 3 must be pinned in ONE test using the SAME stdout, differing only in
   `exited_zero`. They are the pair.
5. `bench` on `{"ms":12.5}` yields `ms: 12.5` and `metrics: Null`.
6. `bench` on stdout with log lines before the JSON reads the JSON. Pin it.
7. `bench` on `{"ms":-1}`, `{"ms":NaN}`-shaped input, or an infinite value yields
   `Err(NotAMeasurement)`. Negative, NaN and infinite are each pinned — a negative millisecond
   is not a fast kernel.
8. `ms: 0.0` is `Ok`. A zero measurement is a measurement; whether it is plausible is
   `task_contract::score`'s business, not this module's.
9. `samples` returns successes in input order and every failure with its INDEX. A set of five
   runs where trials 1 and 3 failed yields three samples and two failures at indexes 1 and 3.
10. `enough` is true exactly when `samples.len() >= m.min_trials`. Pin the boundary at equality,
    and pin that `min_trials: 0` makes an EMPTY sample set enough — the manifest said no minimum
    and this module does not invent one.

## Boundaries, at N and at zero

- Empty stdout with `exited_zero: true`: `Err(Unparseable)` carrying the empty string.
- Stdout that is valid JSON but the wrong shape — `{"correct":true}` with no `detail`, or
  `{"ms":"fast"}`: `Err(Unparseable)`.
- TWO parseable JSON objects on separate lines: the LAST one wins, per the pinned format.
- `samples(&[])`: no samples, no failures. `enough` with `min_trials: 0` is then true and with
  `min_trials: 1` is false.
- `metrics` present but `null`: `Ok`, carrying `Null`. Indistinguishable from absent, and that
  is fine — say so in your handoff.

## Superset status on every enumerated list

`ReadError` has EXACTLY three variants. The fields read are EXACTLY `correct` and `detail` for
verify, `ms` and `metrics` for bench; anything else in the object is ignored, and a test may not
assert on a field this list does not name.

This module runs NOTHING. It does not spawn a process, does not read a file, does not time
anything. Every run arrives as captured stdout plus an exit flag. Spawning here is a defect:
the caller owns the process and its timeout.

## Composition of aggregate returns

`samples` returns a PAIR that partitions the input: every run is either a sample or a failure,
never both and never neither, so `samples.len() + failures.len() == runs.len()`. Pin that on a
mixed input containing one of each `ReadError` variant.

`enough` reads only the sample count and the manifest; it does not look at the failures. A
caller reporting "3 of 5 trials survived, which is not enough" needs both halves and this gives
them separately.
...
- No `unwrap()`, `expect()`, `panic!`, `todo!` or `unimplemented!` reachable from input, outside
  `#[cfg(test)]`.
- Add no dependencies. `serde_json` is already a dependency of this crate.
- Derive `Debug, Clone, PartialEq` on the types this task DEFINES. Not `Eq`: `BenchOutput`
  carries an `f64`. You do not derive anything on the imported types.
