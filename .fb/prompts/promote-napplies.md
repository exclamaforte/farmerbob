...
# Task: refusing to invent evidence is right, and it is not a failure
...
`fb crossx` and `fb critique` now return `4` for "fewer than two candidates" — a not-applicable
state, not a fault, and the normal one under one-arm-per-task. A pipeline that used to halt at
crossx now reaches `promote`, where it halts instead:

```
NO critiques were written -- refusing to emit an empty claims file
```

returning `3`.

The refusal is CORRECT and must not change. An empty claims file would be read downstream as
"the critics found nothing", which is a different fact from "nobody was asked". What is wrong is
the code: `3` is indistinguishable from a genuine failure, so the caller stops. (bead
farmerbob-9ef2)

## Exact API

`run_cmd`'s signature does not change. Its exit codes become:

- `0` — claims were classified and `<bead>.claims.json` was written.
- `1` — a genuine failure: a critique existed and could not be read or processed.
- `4` — **NOT APPLICABLE: no critiques were written, so there is nothing to promote.** Nothing
  went wrong and no claims file is emitted.

The doc comment on `run_cmd` must state all three. `4` matches `fb crossx` and `fb critique`, so
one binary means one thing by one number.

## Falsifiable clauses

1. NO critique files at all: returns 4, and `<bead>.claims.json` is NOT written. Pin the file's
   absence as well as the code — an empty claims file downstream reads as "the critics found
   nothing", which is the confusion this refusal exists to prevent.
2. Critique files exist and contain NO claims: returns 0, and a claims file IS written recording
   zero claims. Pin clauses 1 and 2 as a PAIR — they differ only in whether anybody was asked,
   and collapsing them is exactly the defect. "Nobody was asked" and "asked, and they found
   nothing" are different facts and must not share an answer.
3. Critique files exist and contain claims: unaffected. Whatever it returns today, it returns
   now.
4. A critique file that exists but cannot be READ returns 1, not 4. The file is there; something
   went wrong. Pin it against clause 1.
5. The message printed with 4 says no critiques were written. Assert it mentions critiques;
   do not assert the wording.
6. The `run_cmd` doc comment names all three codes.

## Boundaries, at N and at zero

- ZERO critique files: clause 1, returns 4.
- ONE critique file with zero claims: clause 2, returns 0. This is the boundary and both sides
  must be pinned.
- A critiques DIRECTORY that does not exist at all, versus one that exists and is EMPTY: NOT
  pinned as different. Say in your handoff whether you distinguished them and do not assert on
  it.
- A claims file already present from an earlier run, with zero critiques now: not pinned. Say
  what you did; do not assert.

## Superset status on every enumerated list

The exit codes are EXACTLY 0, 1 and 4. `2` and `3` stay unused here — `fb differential` means
declared-but-unrunnable by 2, `fb crossx` means a failed diagonal invariant by 3, and one binary
giving one number two meanings across subcommands is worse than leaving gaps. Note that this
task RETIRES promote's current use of 3; say in your handoff that you did.

This task changes ONLY the no-critiques path and the doc comment. Claim classification,
contradiction detection, quote verification and the JSON shape are untouched.

## Composition of aggregate returns

`run_cmd` returns ONE code and writes zero or one claims files. Clause 1 is the pairing every
downstream stage depends on: a 4 means no claims file was written this run, so `prove` must not
read a stale one as current.
...
- No `unwrap()`, `expect()`, `panic!`, `todo!` or `unimplemented!` reachable from input, outside
  `#[cfg(test)]`.
- Add no dependencies.
- Change `crates/fb/src/promote.rs` and NOTHING else. `fb-pipeline.sh` and `fb-prove.sh` will
  still treat 4 as a failure until they are updated; **that is EXPECTED and you must leave it.**
