<!-- fb:creates crates/farmerbob-core/src/run_timing.rs -->
# Task: time-to-first-write is measured with stat and arithmetic in awk

Rust workspace, already builds. Work only inside `crates/farmerbob-core`.
Create `crates/farmerbob-core/src/run_timing.rs` and add `pub mod run_timing;` to
`crates/farmerbob-core/src/lib.rs`. Do not change any other file.

## Why this exists

`fb-timing.sh` reports, per candidate, how long an arm took to write its first byte, how long
it kept writing, and how many lines per minute that came to. It does it with `stat -c %Y` over
every changed file and arithmetic in awk, and it is the only place those three numbers exist.

They matter because wallclock alone cannot separate an arm that thought for twenty minutes and
then wrote from one that wrote immediately and then stalled -- and this project has seen both.
`farmerbob_core::timing` already holds a `Timing`/`Mark` type for a different purpose; this is
the per-run derivation the shell does, and it belongs beside it rather than in awk.
...
1. `ttfw_s` is the EARLIEST `mtime_s` minus `started_s`. Pin it with files given out of order,
   because the caller lists a directory and a directory has no order.
2. `span_s` is the latest `mtime_s` minus the earliest.
3. `lines_per_min` is `lines * 60 / span_s` as an `f64` when `span_s > 0`.
4. `lines_per_min` is `Missing` when `span_s == 0`, and the reason says so. One instant of
   writing is not a rate, and dividing by zero to get infinity would rank it best.
5. `derive` returns `Missing` when `wrote` is EMPTY, and the reason says nothing was written.
6. A file whose `mtime_s` is BEFORE `started_s` yields a NEGATIVE `ttfw_s`, preserved verbatim.
   It means the caller's start time is wrong and hiding it behind a clamp to zero would make a
   broken clock look like an instant writer. Say so in your handoff.
7. `lines` is passed through unchanged, including zero.

## Boundaries, at N and at zero

- `wrote` EMPTY: `Missing`. Clause 5.
- `wrote` with ONE file: `span_s` is 0, so `lines_per_min` is `Missing` by clause 4 while
  `ttfw_s` and `lines` are still observed. Pin that the outer value is `Observed` and only the
  rate is absent.
- TWO files with the SAME `mtime_s`: also `span_s == 0`. Same answer, and pin it separately,
  because one file and two simultaneous files are different inputs to the same rule.
- `lines == 0` with a positive span: `lines_per_min` is `Observed(0.0)`. Zero lines per minute
  is a rate and is not the same as having no rate.
- `started_s` exactly equal to the earliest `mtime_s`: `ttfw_s == 0`, observed. Instant is a
  measurement.
...
The fields of `RunTiming` are a CLOSED set of exactly four. Adding a fifth is a defect and your
tests may not assert on anything outside them.

## Composition of aggregate returns

`RunTiming` is one value derived from one list; `ttfw_s`, `span_s` and `lines_per_min` are three
readings of the SAME timestamps and must never come from different subsets of `wrote`. A file
excluded from the earliest calculation and included in the latest would produce a span that no
pair of files supports.

Say why clauses 4 and 5 must be tested together: both return `Missing`, at different levels --
clause 5 makes the whole value absent, clause 4 makes only the rate absent while the rest is
observed -- and an implementation that collapses them into one answer passes either test alone.

## Rules

- No `unwrap()`, `expect()`, `panic!`, `todo!` or `unimplemented!` reachable from input, outside
  `#[cfg(test)]`.
- Add no dependencies, no I/O and no clock. The caller stats the files.
