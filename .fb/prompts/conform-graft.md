<!-- fb:creates crates/farmerbob-core/src/conform.rs -->
# Task: the conformance suite is grafted by appending a file to lib.rs

Rust workspace, already builds. Work only inside `crates/farmerbob-core`.
Create `crates/farmerbob-core/src/conform.rs` and add `pub mod conform;` to
`crates/farmerbob-core/src/lib.rs`. Do not change any other file.

## Why this exists

`fb-conform.sh` runs a hidden conformance suite against every candidate: it copies the
worktree, appends the suite file to `lib.rs`, runs `cargo test`, and reports pass or fail per
arm. The decision inside it -- how many tests were expected, how many ran, and what that means
-- is arithmetic done with `grep -c` and a shell `if`.

It matters because conformance is criterion 1, the first thing ranked, and because the suite is
appended blind: if the graft silently drops tests, the arm is credited with passing a suite that
never ran. That is the same shape as the defect that made an empty `cargo test` read as success.
...
pub enum Conformance {
    ...
    /// Fewer tests executed than the suite declares. The suite did not run
    /// as written and the result says nothing about the arm.
    Short {
...
///
/// `compiled` is whether the grafted copy built at all.
pub fn read(compiled: bool, ran: &Ran) -> Conformance;

/// The fraction of arms that reached `Full`, or `Missing` when no arm was
/// graftable -- which is a fact about the harness, not about the field.
pub fn rate(results: &[Conformance]) -> Measurement<f64>;
```

## Falsifiable clauses

1. `compiled == false` is `NotGrafted`, whatever the counts say.
2. `executed == expected` and `failed == 0` is `Full`.
3. `failed > 0` is `Broke`, even when `executed == expected`.
4. **`executed < expected` is `Short`, never `Full` and never `Broke`**, even when
   `failed == 0`. A suite that lost tests in the graft did not pass; it did not run.
5. `executed > expected` is also `Short`, with both numbers carried verbatim. More tests than
   the file declares means the count and the graft disagree, and guessing which is right is how
   a silent miscount survives.
6. `rate` counts `Full` over arms that are NOT `NotGrafted`. A `NotGrafted` arm is excluded from
   the denominator entirely, the way `Cell::Error` already is in cross-examination.
7. `rate` is `Missing` when every arm is `NotGrafted`, with a reason saying so. Zero over zero
   is not zero.

## Boundaries, at N and at zero

- `expected == 0` and `executed == 0` with `compiled == true`: `Short`, not `Full`. A suite
  declaring no tests cannot certify anything, and this is the empty-suite-exits-zero defect in
  its conformance clothing. Pin it.
- `results` EMPTY: `rate` is `Missing`. There is no field to take a fraction of.
- Every arm `Full`: `rate` is `Observed(1.0)`.
- Every arm `Broke`: `rate` is `Observed(0.0)`. Zero passing is a measurement; clause 7's
  absence is a different case and both must be pinned.
- ONE arm `Full` and ONE `NotGrafted`: `rate` is `Observed(1.0)`, because the denominator is
  one. Pin it, because folding the ungraftable arm into the denominator would report 0.5 and
  blame a field for a harness failure.

## Superset status on every enumerated list

`Conformance`'s variants are a CLOSED set of exactly four: `Full`, `Broke`, `Short`,
`NotGrafted`. Adding a fifth is a defect, and your tests may not assert on anything outside
them.
...
`rate` returns one fraction over a denominator that excludes `NotGrafted` and includes every
other variant exactly once. State in your handoff what the denominator was for each test you
wrote, because a rate whose denominator is unstated is the thing this module exists to make
explicit.

Say why clauses 4 and 6 must be tested together: clause 4 keeps a short run out of `Full`, and
clause 6 keeps an ungraftable arm out of the denominator. An implementation that confuses the
two reports a field as conformant because the arms that would have failed never ran.

## Rules

- No `unwrap()`, `expect()`, `panic!`, `todo!` or `unimplemented!` reachable from input, outside
  `#[cfg(test)]`.
- Add no dependencies and no I/O. The caller runs cargo; this reads the numbers.
