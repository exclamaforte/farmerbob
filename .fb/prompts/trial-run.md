<!-- fb:creates crates/farmerbob-core/src/trial_plan.rs -->
# Task: the benchmark loop has a reader and a scorer and no plan between them

Rust workspace, already builds. Work only inside `crates/farmerbob-core`.
Create `crates/farmerbob-core/src/trial_plan.rs`. Declare it in `lib.rs` with one
`pub mod trial_plan;` line and change nothing else.

## Why this exists

The benchmark path now has both ends and no middle:

- `bench_read` turns one run's captured stdout into a `VerifyOutput` or a `BenchOutput`.
- `task_contract::score` turns a verification plus samples into a `Score`.
- `gate_kind` turns that `Score` into a gate decision.

What decides HOW MANY trials to run, whether to keep going after a failure, and when to stop
early does not exist. A caller would have to invent it, and the obvious inventions are wrong in
ways this project has already paid for: running a fixed count regardless of failures, or
stopping at the first bad trial and reporting the kernel as incorrect when the harness flaked.
(bead farmerbob-p172)

This module is that decision, as a pure function of what has happened so far.
...
`TaskManifest` is `farmerbob_core::task_contract`'s; `min_trials` is the field this reads. You do
not define it.

## Falsifiable clauses

1. `good < min_trials`, `bad` at or below `max_bad`, `attempted < max_attempts`: `Run`.
2. `good >= min_trials`: `Enough(good)`, even when `attempted < max_attempts`. Having enough is a
   reason to stop, not merely permission to.
3. `bad > max_bad`: `Abandon(bad)`, EVEN IF `good >= min_trials`. Pin this against clause 2 with
   the same `good`: an instrument that failed more than tolerated is suspect, and a result
   gathered alongside that many failures must not be scored. This is the clause that carries the
   task.
4. `attempted >= max_attempts` with `good < min_trials` and `bad` within tolerance:
   `Abandon(bad)`. Running out of attempts without enough measurements is an instrument problem,
   not a candidate failure — the candidate was never measured.
5. Clauses 2 and 4 must be pinned as a PAIR: the same `attempted` and `max_attempts`, differing
   only in `good`. One is `Enough`, the other `Abandon`, and an implementation that returns the
   same thing when attempts run out passes clause 2 alone.
6. `scorable` is true for `Enough` and FALSE for both `Run` and `Abandon`. Pin all three —
   `Abandon` being unscorable is the point, because scoring an abandoned run blames a candidate
   for the harness.
7. The decision reads `min_trials` from the manifest and nothing else from it. Pin that two
   manifests differing only in `name` or `description` decide identically.

## Boundaries, at N and at zero

- `min_trials: 0` with `good: 0`: `Enough(0)`. The manifest asked for no minimum and this module
  does not invent one. Pin it — and say in your handoff whether you think a zero-sample score is
  meaningful, because that judgement belongs to `task_contract::score`, not here.
- `good` EXACTLY equal to `min_trials`: `Enough`. The boundary is at equality, not above it.
- `bad` EXACTLY equal to `max_bad`: NOT abandoned — tolerance means the limit is allowed. Pin
  both sides, at `max_bad` and at `max_bad + 1`.
- `attempted` exactly equal to `max_attempts`: no further `Run`. Pin at equality.
- `max_attempts: 0`: `Abandon` immediately, whatever else holds. Nothing may be attempted.
- `max_bad: 0`: a single bad run abandons. Pin it.

## Superset status on every enumerated list

`Next` has EXACTLY three variants. Closed set.

This module runs nothing, reads no clock, and does not decide whether a MEASUREMENT is good —
`bench_read` already did that and `Progress` is its tally. Counting stdout or inspecting a
sample here is a defect.

Note that `attempted` is not required to equal `good + bad`: a caller may attempt a trial and
record neither if it is still running. Do NOT assume the identity, and say in your handoff
whether your implementation would behave differently if it held.

## Composition of aggregate returns

`next` returns exactly one `Next`, and `scorable` reads it — clause 6 is the statement that
`Abandon` and `Enough` are not both endings of the same kind. One yields a score; the other
yields a report that the instrument failed, and the difference must survive to the caller.

## Rules

- No `unwrap()`, `expect()`, `panic!`, `todo!` or `unimplemented!` reachable from input, outside
  `#[cfg(test)]`.
- Add no dependencies. No filesystem, no process spawning, no clock.
