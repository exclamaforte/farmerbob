<!-- fb:creates crates/fb/src/compare_cmd.rs -->
# Task: fb-compare.sh is 47 lines of shell over a core module that already exists

Rust workspace, already builds. Work only inside `crates/fb`.
Create `crates/fb/src/compare_cmd.rs`. Declare it in `main.rs` with one `mod compare_cmd;` line
and change nothing else. `fb` is a binary crate with no `lib.rs`, so `main.rs` is the only place
that line can go.

## Why this exists

`farmerbob_core::compare` is merged and tested and reachable from nothing. `fb-compare.sh` is
still the only caller-shaped thing in the tree, and it is shell.

This is one of the last four scripts with no Rust behind it. The others are being ported in
their own tasks; this one is small enough to be a clean port, which is why it is here.
...
`Measurement` is `farmerbob_core::measurement::Measurement`. `Write` is `std::io::Write`.
...
1. A single row with all three figures `Observed` renders one data line containing the arm name
   and all three numbers.
2. A `Missing` figure renders as `ABSENT`, the literal `"--"`. This constant is quoted verbatim
   by the spec, so it IS pinned and your tests may assert it exactly.
3. `Observed(0)` renders as `0`, NOT as `ABSENT`. Pin clauses 2 and 3 as a PAIR against rows
   differing only in that one field: a measured zero and an unread figure are different facts,
   and collapsing them is the defect this whole crate exists around.
4. Every row in the input appears in the output, in input order, exactly once.
5. The table has a heading line naming the four columns. Assert it MENTIONS each column name
   case-insensitively; do NOT assert the layout, the padding, or the separator, none of which
   this spec fixes.
6. `run` writes exactly what `table` returns and returns 0 for a non-empty input.
7. `run` returns 1 for an empty input and still writes the heading. It does not write nothing.
8. Rows are not sorted, deduplicated or filtered. Pin with two rows sharing an arm name: both
   appear.

## Boundaries, at N and at zero

- ZERO rows: clause 7. Heading only, exit 1.
- ONE row: exit 0.
- A row where ALL THREE figures are `Missing`: rendered, with three `ABSENT` markers. It is
  still a candidate and dropping it would hide an arm that produced nothing measurable.
- An empty arm name: not pinned. Say what you did in your handoff and do not assert on it.
- `Observed(u32::MAX)`: renders as the number. Pin that nothing overflows or truncates.

## Superset status on every enumerated list

The columns are EXACTLY arm, tests, lines, clippy, in that order. Adding a fifth is a defect:
`fb brief` is where more evidence belongs, and two tables that disagree about what a comparison
contains is worse than one table with four columns.

The exit codes are EXACTLY 0 and 1.

## Composition of aggregate returns

`table` returns the heading plus one line per row, in input order — clause 4 — and clause 8
states it does not touch the multiset. `run` writes that string and returns a code that says
only whether there were rows.

## Rules

- No `unwrap()`, `expect()`, `panic!`, `todo!` or `unimplemented!` reachable from input, outside
  `#[cfg(test)]`.
- Add no dependencies. No filesystem: every row arrives as a value.
- Do NOT wire the subcommand into the argument parser: that would change `main.rs` beyond the
  one `mod` line. Wiring is a separate task. Say so in your handoff, and use
  `#![allow(dead_code)]` with a comment if `-D warnings` needs it — an uncalled `pub fn` in a
  BINARY crate trips `dead_code`, which has already cost one arm a clean clippy score.
- Derive `Debug, Clone, PartialEq, Eq` on `Row`. This is pinned because a suite asserting with
