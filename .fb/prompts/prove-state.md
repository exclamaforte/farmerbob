<!-- fb:modifies crates/fb/src/prove.rs -->
# Task: fb prove is the last path running agents on a shared database

Rust workspace, already builds. Work only inside `crates/fb`.
Modify `crates/fb/src/prove.rs`. Do not change any other file.

## Why this exists

Three implementers once hung, holding one `~/.local/share/opencode/opencode.db` open between
them. The diagnosis was concurrency against a single SQLite database, not the network and not
the cgroup. (bead farmerbob-p3r)

Two of the three agent-launching paths are now isolated. `fb-dispatch.sh` gives every run its
own `XDG_DATA_HOME`/`XDG_STATE_HOME`/`XDG_CACHE_HOME`, and `fb critique` was fixed in
`critique-state` — the arm that did it noted in its handoff that `fb prove` has the identical
defect and left it alone, because the spec scoped it to one file.

This is that file. `fb prove` launches a prover per subject, concurrently, and sets none of
those variables, so every prover shares the same store — currently 275MB.

## Exact API

Whatever function in this file spawns the prover gains the run identity it needs and sets three
environment variables on the `Command` before spawning:
...
where `<state_root>` is `$HOME/.local/share/farmerbob/state`, matching `fb-dispatch.sh` and
`critique.rs`.

**Look at how `critique.rs` does it and do the same thing.** It landed first, it separates the
path computation from the spawn so the computation can be tested, and two implementations of
one rule in one crate is the duplication this project keeps paying for. If you think its shape
is wrong, say so in your handoff; do not quietly invent a second one.

`run` is the per-prover identity: unique per (task, subject) pair and stable across a retry. Its
exact spelling is not pinned — say what you chose.

## Falsifiable clauses

1. Two spawns with DIFFERENT `run` values receive different `XDG_DATA_HOME` values. Pin that
   they differ; do not pin the strings.
2. Two spawns with the SAME `run` receive the same value. Stable, not random per call.
3. All three variables are set, and all three point inside that run's directory. Pin that
   `XDG_STATE_HOME` and `XDG_CACHE_HOME` are not left to inherit — inheriting is the current
   behaviour and it is the defect.
4. The three directories exist before the command runs.
5. Two DIFFERENT subjects of the same task get different directories. Pin it — a task proving
   claims against three implementations launches three provers concurrently, and that is the
   fan-out this fixes.
6. `fb prove`'s exit codes are unchanged: 4 when there is no claims file, and whatever it
   returns today otherwise. Pin the 4 case, which `fb-prove.sh` and the pipeline both depend on.
7. Nothing else about proving changes: which claims are selected, the reference veto, the
   CONFIRMED/refuted counts. Pin one existing case.

## Testing this

Spawning a real prover in a test is not acceptable — it costs money and needs the network.
Compute the environment in a function you can call directly and keep spawning separate, as
`critique.rs` does. Say in your handoff what remains untested because it only happens at spawn
time.

## Boundaries, at N and at zero

- ZERO subjects: nothing is spawned and nothing is created. Pin that no directory appears for a
  task with no claims.
- An EMPTY `run` string: not pinned. Say what you did.
- A `run` containing a path separator: say in your handoff whether you rejected, sanitised or
  passed it through — a `..` there escapes the state root, so this is worth stating even though
  the spec does not pin it.

## Superset status on every enumerated list

EXACTLY three environment variables. Adding `HOME` or `TMPDIR` is out of scope; say so in your
handoff instead.

This task does not touch `critique.rs`, `fb-launch.sh` or `fb-dispatch.sh`.

## Composition of aggregate returns

The isolation is a side condition on the environment each child receives. `fb prove`'s return
value and its written artefacts are unchanged — clauses 6 and 7 are that statement.
...
- No `unwrap()`, `expect()`, `panic!`, `todo!` or `unimplemented!` reachable from input, outside
  `#[cfg(test)]`.
- Add no dependencies.
- Change `crates/fb/src/prove.rs` and NOTHING else.
