...
# Task: the scorer reports the crate's test count as if the candidate wrote it
...
`score.json` records `tests_run` as the number of tests passing in the WHOLE CRATE, and that
figure is printed beside a candidate's name in a column called tests.

Measured on a merged task: `sem-port`'s arm wrote SIX tests. Its record says `tests_run: 547`.
The crate had 541 before it. Every `tests=NNNN` line in this project's wave logs is a crate
total, and they have been quoted in adjudications as though they described the arm.
(bead farmerbob-apxp)

`farmerbob_core::test_delta` was merged to fix this and has no caller:

```rust
pub struct Counts { pub before: Measurement<u32>, pub after: Measurement<u32> }
pub enum Contribution { Added(u32), Removed(u32), Unchanged }
...
The record gains one field beside `tests_run`, which is unchanged:

```rust
/// What this candidate contributed to the crate's suite, as
/// `farmerbob_core::test_delta` decides it. `Missing` when the baseline
/// could not be measured -- never a number standing in for one.
tests_delta: Measurement<Contribution>,
```

`tests_run` KEEPS its current meaning and its current name. It is the crate total, it is
honest about being one now that `tests_delta` sits beside it, and other tools read it.

The scorer already measures the crate's passing count after a run. It must also obtain the
BASELINE — the count on the run's base — and pass both to `test_delta::contribution`. Where that
baseline comes from is yours to choose; state it in your handoff. If it cannot be obtained for a
given candidate, `before` is `Missing` and the contribution is `Missing`. That is the correct
answer, not a failure.

## Falsifiable clauses

1. A candidate whose crate went from 541 to 547 passing records
   `tests_delta: Observed(Added(6))` and `tests_run: 547`. Both fields, one record.
2. A candidate whose baseline could not be measured records `tests_delta: Missing` and STILL
   records its `tests_run`. Pin that a missing delta does not suppress the total — they answer
   different questions and `test_delta::crate_total` exists to say so.
3. Clauses 1 and 2 must be pinned with the SAME `after` value, differing only in whether the
   baseline was measured. 547 with a known baseline is "added 6"; 547 with an unknown baseline
   is unknown, and it is never "added 547".
4. A candidate that BREAKS existing tests — crate total lower than the baseline — records
   `Removed(n)`, not `Unchanged` and not `Added(0)`. Pin it; an implementation using saturating
   subtraction hides a regression here.
5. The decision comes from `test_delta::contribution`. Assert agreement with it rather than
   re-deriving; a subtraction in this file is the defect this task exists to remove.
6. `tests_run` is unchanged for every existing case. Pin one existing scored record's
   `tests_run` before and after this change.
7. The JSON written to `<task>.score.json` gains the field and loses none. Pin that a reader of
   the old fields still finds them.

## Boundaries, at N and at zero

- Baseline and after both zero: `Unchanged`, and `tests_run: 0`. A crate with no tests is an
  observation.
- A candidate that is a NO-OP — wrote nothing: not pinned by this task. Say in your handoff what
  its delta came out as and whether that is meaningful.
- A candidate whose own suite could not be run at all: `after` is `Missing`, so the contribution
  is `Missing`. Pin it.

## Superset status on every enumerated list

Exactly one field is added. No existing field changes name, type or meaning.

This module does NOT count tests itself for the delta. `farmerbob_core::testout` parses cargo
output and `test_delta` does the arithmetic; re-deriving either here is a defect.

## Composition of aggregate returns

A record now carries both the candidate's contribution and the suite it sits in, and clause 2 is
the statement that one being absent does not suppress the other. A reader that shows only the
total is the bug this task makes fixable.
...
- Tests that need a git repository create one in a temporary directory. Do NOT read this
  repository's own worktrees; they are mutable state shared with a running harness.
- Change `crates/fb/src/score.rs` and NOTHING else.
