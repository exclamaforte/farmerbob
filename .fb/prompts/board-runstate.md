<!-- fb:modifies crates/farmerbob-core/src/board.rs -->
# Task: the board cannot say "the runs are done, go score them"

Rust workspace, already builds. Work only inside `crates/farmerbob-core`.
Modify `crates/farmerbob-core/src/board.rs`. Do not change any other file.

## Why this exists

`fb next` was wired and used, and on its first tick it answered `QueueEmpty` — nothing to do —
while a task's runs had finished minutes earlier and sat unscored. Running the scorer made the
item appear at once.

`observe` yields a tier item for a task only when its passing count was actually OBSERVED. That
rule is right and must not be weakened: a task whose score file is unreadable must never be
reported as having zero passing candidates. But `TaskFiles` says nothing about whether the RUNS
have finished, so two opposite situations are identical to the board:

- dispatched and still live — worktrees exist, no score — **wait**
- finished, never scored — worktrees exist, no score — **score it**

and "the runs are done, go score them" is the most common thing the orchestrator needs to be
told. (bead farmerbob-ervw)

`farmerbob_core::run_state_view` was merged to decide exactly this and has no caller. This task
gives it one.

## Exact API

`TaskFiles` gains two fields, and nothing else about it changes:

```rust
/// How many worktrees exist for this task, of any arm.
pub worktrees: usize,
/// How many agents for THIS task are running right now. `Missing` when the
/// caller could not ask.
...
`observe`'s signature does not change.

## Falsifiable clauses

1. A task whose `run_state_view::state` is `FinishedUnscored` yields `ScoreIt`, carrying the
   task name and the worktree count unchanged.
2. It yields `ScoreIt` INSTEAD OF, not in addition to, the tier items — a task with no observed
   passing count yields no `Adjudicate` and no `RunPipeline` today, and that does not change.
3. A task whose state is `Running` yields no `ScoreIt`. Pin clauses 1 and 3 against `TaskFiles`
   differing ONLY in `live` — they are the two the board confuses and they are opposite
   instructions.
4. A task whose state is `Unknown` — `live` is `Missing` — yields no `ScoreIt`. An unasked
   question is not a negative answer, and a false `ScoreIt` would have the orchestrator measure a
   live run out from under itself.
5. A task whose state is `Scored` behaves EXACTLY as it does today: the existing clauses decide,
   and `ScoreIt` never appears. Pin one already-passing case from the existing suite and assert
   its items are unchanged.
6. `NeverDispatched` yields no `ScoreIt`.
7. The decision comes from `run_state_view::state`. Assert agreement with it for every input
   rather than re-deriving the rule; a second copy of the worktree-and-live logic in `board.rs`
   is the defect this task exists to avoid.
8. `PipelineFailed` still comes first and is still independent: a task with failed stages AND a
   `FinishedUnscored` state yields both, `PipelineFailed` first. The module doc already says a
   finished decision does not repair a broken instrument.
9. Every existing test in `board.rs` still passes, unchanged. If one cannot, say which in your
   handoff and why the spec forced it, rather than editing it quietly.

## Boundaries, at N and at zero

- `worktrees: 0` with `live: Observed(0)` and no score: `NeverDispatched`, so no `ScoreIt`.
- `worktrees: 1` with `live: Observed(0)` and no score: `ScoreIt { worktrees: 1 }`. One worktree
  is a field worth scoring.
- ZERO tasks: unchanged — the existing empty-listing behaviour, including `QueueEmpty`.
- A task that is BOTH `FinishedUnscored` and already adjudicated: not pinned. Say in your
  handoff which you returned and do not assert on it.

## Superset status on every enumerated list

`Item` gains EXACTLY one variant. `TaskFiles` gains EXACTLY two fields.

`attention::rank` orders items by variant, and `ScoreIt` is NEW to it. This task does not touch
`attention.rs`, so `rank`'s treatment of an unknown variant is whatever it already does — say in
your handoff what that turned out to be, and do NOT change `attention.rs` to suit: that is a
second file and a scope violation. If `rank` cannot order the new variant at all, that is a
finding for the handoff, not something to fix here.

## Composition of aggregate returns

`Facts.items` keeps its existing composition — tasks in the order given, then queued waves, then
`QueueEmpty` when the listing left nothing to do — with `ScoreIt` taking the place the tier items
would have had for that task. Pin that a three-task listing where the middle one is
`FinishedUnscored` still yields items for all three, in order.

## Rules

- No `unwrap()`, `expect()`, `panic!`, `todo!` or `unimplemented!` reachable from input, outside
  `#[cfg(test)]`.
- Add no dependencies. No filesystem and no clock, as the module doc already requires.
- Change `crates/farmerbob-core/src/board.rs` and NOTHING else. This is a `modifies` task: no new
  file, so no module declaration to add. Adding the `worktrees` and `live` fields will break
  every caller that constructs a `TaskFiles` — `crates/fb/src/next_cmd.rs` does. **That breakage
  is EXPECTED and you must leave it.** Reaching out to fix it is the departure. Say in your
  handoff that you left it broken.
- Derive nothing new; `TaskFiles` and `Item` already derive what they need.
- RUN `cargo clippy -p farmerbob-core --all-targets -- -D warnings` BEFORE you finish, including
  over your tests. `cargo test -p farmerbob-core` must pass. The wider workspace will NOT build;
