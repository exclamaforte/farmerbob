<!-- fb:modifies crates/fb/src/decl_cmd.rs -->
# Task: a refused marker must say what is wrong with it

Rust workspace, already builds. Work only inside `crates/fb`.
Modify `crates/fb/src/decl_cmd.rs`. Change no other file.

## Why this exists

`fb decl` is the one reader for `<!-- fb:creates PATH -->` and
`<!-- fb:modifies PATH -->`. It decides whether a task can be dispatched at all, and when it
refuses a marker it prints NOTHING:

```text
<!--fb:creates a.rs -->                  -> rc=3   (silent)
<!-- fb: creates a.rs -->                -> rc=3   (silent)
<!-- fb:creates a.rs --> note            -> rc=3   (silent)
<!-- fb:creates a.rs -->                 -> rc=0   creates a.rs
```

Each of those three is refused for a DIFFERENT, precisely known reason.
`farmerbob_core::precondition::Rejected` names them — `NoSpaceAfterOpener`, `Tab`,
`TrailingText`, `UnknownWord`, `MultipleMarkers` — and `NoDeclaration::Refused` carries the
offending line number. None of it reaches the person who wrote the spec.

So an author cannot tell "my marker is malformed, here is how" from "this spec declares
nothing". `<!--fb:creates a.rs -->` is valid HTML that a careful person would write, and the
answer they get is silence.

This is the root of bead farmerbob-t8a5. Four arms once split into two camps over what a
marker IS, and six independent readers disagreed, because nothing ever showed what the reader
was refusing or why.

## The contract you must not break

`run`'s doc says: *"Nothing is written to `out` for any non-zero code, so a caller that
captures the output and one that checks the exit code get the same answer."* That is
deliberate — `out` is for a machine to consume — and it stays true. **Diagnosis goes to a
separate stream.** Exit codes do not change.
...
///
/// `out` receives the answer and nothing else. `why` receives a human-readable diagnosis
/// when, and only when, the answer is a refusal.
...
/// The diagnosis for one refusal: what is wrong, on which line, and what to write instead.
///
/// Returns `None` for a `NoDeclaration` that is not a refusal — an absent declaration is not
/// a malformed one, and reporting it as an error would make every note and template in
/// `.fb/prompts` look broken.
pub fn diagnose(reason: &NoDeclaration) -> Option<String>;
```

`run` keeps its exact signature and behaviour, and calls `run_explained` with a sink that
discards, so every existing caller is unaffected.

## Falsifiable clauses

1. A refused marker writes a diagnosis to `why` and NOTHING to `out`. Pin `out` empty
   explicitly; a caller that captures stdout must see no change.

2. The diagnosis names the 1-based LINE the marker is on. `NoDeclaration::Refused` carries
   it; a reader with a 400-line spec cannot use a message that does not.

3. The diagnosis names the REASON in words a spec author can act on, not the enum's Debug
   form. Pin one string per `Rejected` variant, matched case-insensitively on a
   distinguishing phrase:
   - `NoSpaceAfterOpener` — must mention a space after `<!--`
   - `Tab` — must mention a tab
   - `TrailingText` — must mention text after `-->`
   - `UnknownWord` — must mention `creates` and `modifies`
   - `MultipleMarkers` — must mention one marker per line

   This list is a SUPERSET: `Rejected` may gain variants, and a variant this function does
   not know must still produce SOME diagnosis naming the line, never an empty string and
   never a panic. Pin that with a wildcard arm. (bead farmerbob-2ve)

4. The diagnosis shows what a correct marker looks like, verbatim, so the fix needs no other
   document.

5. `NoDeclaration::Absent` yields `None` from `diagnose`, and `run_explained` writes nothing
   to `why`. A spec that declares nothing is not a spec that got it wrong.

6. `NoDeclaration::Ambiguous` also yields `None`. Since 2026-09-19 a task may declare several
   deliverables, so several markers is not a fault — `declared` still reports it for callers
   that need exactly one, and that is their business, not a malformation.

7. Exit codes are UNCHANGED: 0 one declaration, 2 none, 3 refused or ambiguous, 4 unreadable
   file. Pin all four against `run_explained`.

## Boundaries, at N and at zero

- An UNREADABLE FILE (code 4) writes nothing to `why`: the failure is about the file, not
  about a marker, and a marker diagnosis there would name a line nobody wrote.
- An EMPTY spec: `Absent`, code 2, `why` empty.
- A spec whose ONLY marker is inside a fenced code block: `Absent`, code 2, `why` empty. It
  is an example, and examples are not refusals.
- Exactly ONE refusal is reported even if the spec contains several malformed markers:
  `NoDeclaration::Refused` carries one line, and inventing a list from a value that holds one
  would be reporting more than was measured.

## Composition of the aggregate return

`run_explained` returns exactly one exit code, and writes to at most one of the two streams:
`out` on success, `why` on refusal, neither on absence or an unreadable file. It never writes
to both.

## Rules

`diagnose` is pure: no file system, no clock, no environment. This module parses NOTHING —
"a regex or a scan here would be the fourth reader this subcommand exists to retire", and
that is still true; every fact comes from `NoDeclaration`. Every clause above gets a test
named for what it pins. Keep the workspace rustfmt-clean and clippy-clean and every existing
test passing.
