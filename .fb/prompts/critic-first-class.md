...
<!-- fb:modifies crates/fb/src/adjudicate_cmd.rs -->
# Task: a critic is a first-class run, not a second-class one

Rust workspace, already builds. Work only inside `crates/fb`.
Modify `crates/fb/src/critique.rs` and `crates/fb/src/adjudicate_cmd.rs`. Change no other file.

## Why this exists

This task closes five bugs at once, because they are one defect seen five ways: **a critic
gets none of the protections an implementer gets, and none of the reporting.**

An implementer is launched as:

```rust
launch::launch_in(arm, prompt, wd, continue_session, &env, Some(Scope {
    unit, memory_max, runtime_max_s,          // systemd scope: cap, timeout, visibility
}))
```

A critic is launched as `launch::launch(arm, text, worktree, false, &env)` --
`crates/fb/src/critique.rs:285` -- with **no `Scope` at all**. Everything below follows from
that, or from the same second-class treatment of what a critic produces.

  farmerbob-vi7   Critics run unconfined: no systemd scope, no memory cap.
  farmerbob-epb5  No timeout. One wandering critic runs for ever, and is invisible to
                  admission control because it has no scope to be counted in.
  farmerbob-kk8w  A rate-limited critic is not parked and not retried, and the report says
                  "no critique written" -- the same line as a critic that ran and had
                  nothing to say.
  farmerbob-b7c5  `fb brief` renders a critique as blank when it filed no CLAIMS, hiding its
                  JUDGEMENTS and FOLLOWUPS entirely.
  farmerbob-fp7q  The critic is shown only the FIRST declared target, so a task declaring
                  several files gives it a header to review and hides the rest.

Each of these was paid for on 2026-09-19. Two critics hit an account-wide provider quota
twelve minutes apart; neither was parked, both reported "no critique written", and two tasks
lost their subjective tier. Separately, critics running without scopes is exactly why the
harness's agent count missed them: `fb live` had to be rewritten to read `/proc` because
scope-counting could not see a critic at all.
...
///
/// This list is CLOSED: a sixth case is a change to this enum, not an extension.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum NoReview {
    /// The critic ran and chose to write nothing.
    Declined,
    /// The provider refused the critic. Carries the refusal text, verbatim.
    Refused { evidence: String },
    /// The critic outlived its timeout and was killed.
    TimedOut { after_s: u64 },
    /// The launcher could not start it at all. Carries the reason.
    NotLaunched { reason: String },
    /// A review file exists but is empty.
    Empty,
}

/// Run one critic under the same confinement an implementer gets.
///
/// Returns the review text, or why there is none. NEVER returns an empty `Ok`: an empty
/// review is `NoReview::Empty`, because "wrote nothing" and "was refused" are different
/// facts about different things and the caller reports them differently.
pub fn run_critic(arm: &str, prompt: &str, worktree: &Path, timeout_s: u64)
    -> Result<String, NoReview>;

/// Every declared deliverable's patch, concatenated, each under a header naming its path.
///
/// `patch_for` takes ONE target. A task may declare several since 2026-09-19.
pub fn patches_for(worktree: &Path, targets: &[String]) -> Measurement<String>;

/// The sections a critique file carries, whether or not it filed any CLAIMS.
#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub struct Sections {
    /// The CLAIMS section's body, empty when absent.
    pub claims: String,
    /// The JUDGEMENTS section's body, empty when absent.
    pub judgements: String,
    /// The FOLLOWUPS section's body, empty when absent.
    pub followups: String,
}

/// Split a critique into its sections. In `adjudicate_cmd`.
pub fn sections_of(body: &str) -> Sections;
```

## Falsifiable clauses

1. `run_critic` launches through `launch::launch_in` with `Some(Scope { .. })`, so a critic
   gets a systemd scope, a memory cap and a runtime cap exactly as an implementer does. Pin
   that the `Scope` passed is non-`None` and that `runtime_max_s` equals the `timeout_s`
   given. (vi7, epb5)

2. A critic that exceeds `timeout_s` yields `NoReview::TimedOut { after_s }` and NOT
   `Declined`. A run that was killed did not decline. (epb5)

3. A critic whose output carries a provider refusal yields `NoReview::Refused { evidence }`
   carrying the refusal text verbatim. Use `farmerbob_core::limit_signal::classify` --
   do NOT write a second refusal detector; that module is the one reader and this project
   has paid four times for a second copy of a decision. (kk8w)

4. `NoReview::Refused` and `NoReview::Declined` render DIFFERENTLY in the caller's report.
   Pin that the rendered line for `Refused` contains the evidence and the word `refused`,
   and that `Declined`'s does not. "No critique written" for both is the defect. (kk8w)

5. `sections_of` returns JUDGEMENTS and FOLLOWUPS even when CLAIMS is absent or empty. Pin
   it with a critique whose body is `### JUDGEMENTS\n...` and no CLAIMS heading at all: the
   result must carry the judgements text. (b7c5)

6. `patches_for` concatenates every declared target's patch, each under a header naming its
...
- `targets` EMPTY: `patches_for` is `Missing`, not an empty string. A task declaring nothing
  gives a critic nothing to review, and an empty patch reads as "reviewed, no changes".
- ONE target: byte-identical to what `patch_for` produced, so nothing about the
  single-deliverable case changes.
- `timeout_s` of ZERO: refuse to launch and return `NoReview::NotLaunched`, rather than
  launching something that is killed immediately -- a critic that never had a chance must
  not be recorded as one that ran.
- A critique file that exists and is EMPTY is `NoReview::Empty`, never `Ok("")`.
- `sections_of` on an empty body returns `Sections::default()`, all three empty, and is not
  an error.
...
`sections_of` and the rendering of a `NoReview` are pure and take no filesystem, clock or
environment. Use `farmerbob_core::limit_signal` for refusal detection rather than writing
one. Every clause above gets a test named for what it pins. Keep the workspace
