...
# Task: an unrecognised refusal must not silently become the narrowest blast

Rust workspace, already builds. Work only inside `crates/farmerbob-core`.
Modify `crates/farmerbob-core/src/quota.rs` and `crates/farmerbob-core/src/park_decision.rs`.
Change no other file.

## Why this exists

`blast_of` decides how many arms one provider refusal should park. It recognises two
phrasings and falls through to the narrowest answer:

```rust
pub fn blast_of(log_head: &str) -> Blast {
    let lowered = log_head.to_lowercase();
    if lowered.contains("key limit") { Blast::Credential }
    else if lowered.contains("token limit") { Blast::Bucket }
    else { Blast::Arm }
}
```

On 2026-09-19 this cost two runs and two critiques, within twelve minutes.

`agy-opus-46` was refused as a critic with *"Individual quota reached. Please upgrade your
subscription to increase your limits. Resets in 3h54m29s."* That phrase matches neither
pattern, so the blast was computed as `Arm`. Only that one arm was parked. The next cast
fell through to `agy-sonnet-46` — which shares `provider = "agy-anthropic"` — and it hit the
same wall at 17:35, reporting `Resets in 3h46m52s`: seven and a half minutes less on the
same countdown, twelve minutes later. One clock, one credential, two burned dispatches.

The defect is not the missing phrase. **It is that an unrecognised refusal is
indistinguishable from a recognised arm-only one.** Both return `Blast::Arm`, so no caller
can tell "this provider limits one arm" from "nobody taught me this vendor's wording", and
the second is the case that keeps recurring: every new vendor phrases refusals differently.
...
///
/// This list is CLOSED: a third case is a change to this enum, not an extension.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum BlastCall {
    /// The wording was recognised and the width is known.
    Recognised(Blast),
    /// A refusal whose wording matches no known pattern. Carries NO `Blast`: the width is
    /// not known, and supplying one here is the defect this type exists to remove.
    Unrecognised,
}
...
/// Every phrasing this module recognises, with the width it implies.
///
/// A SUPERSET, never a closed world: vendors add and reword refusals, and a caller must be
/// able to see what is covered. (bead farmerbob-2ve)
pub fn known_phrasings() -> &'static [(&'static str, Blast)];
```

`blast_of` is REMOVED. Every caller moves to `blast_call`; leaving both would keep the
defaulting version reachable, which is what made this bug survivable.

In `park_decision.rs`, `decide` consumes `BlastCall`. Its `Decision` gains one case:

```rust
    /// A refusal was seen and its wording was not recognised, so how far it reaches is
    /// unknown. Carries the refused arm and the evidence. The caller must NOT park a
    /// credential on this, and must not treat it as `Leave` either: a refusal happened.
    UnknownBlast {
        /// The arm that was refused.
        arm: String,
        /// The matched refusal text, verbatim.
        evidence: String,
    },
```
...
1. `"key limit"` yields `Recognised(Blast::Credential)`; `"token limit"` yields
   `Recognised(Blast::Bucket)`. Both are existing behaviour and must not change.

2. **`"Individual quota reached"` yields `Recognised(Blast::Credential)`.** This is the agy
   phrasing above, and the evidence that it is credential-wide is in this spec: two arms
   sharing one `provider` on one countdown. Match case-insensitively.

3. A log with no refusal marker at all yields `Unrecognised`, NOT
   `Recognised(Blast::Arm)`. Pin it with ordinary successful cargo output.

4. A refusal whose wording matches nothing known yields `Unrecognised`. Pin it with a
   plausible invented refusal, e.g. `"error: monthly allowance consumed"`.

5. `Blast::Arm` is still reachable — `Recognised(Blast::Arm)` — but ONLY from a phrasing
   that genuinely means one arm. Add exactly one such phrasing to `known_phrasings` and
   pin it; `"model is overloaded"` is the intended member, since it is about one model and
   not about a credential.

6. `known_phrasings()` is non-empty, contains every pattern clause 1, 2 and 5 name, and each
   entry's `Blast` is what `blast_call` returns for a log containing that pattern. Pin that
   by iterating the list, not by restating it.

7. `decide` returns `Decision::UnknownBlast { arm, evidence }` when `blast_call` is
   `Unrecognised` AND a refusal was classified. It returns `Decision::Leave` when no refusal
   was classified at all. These are different and must not collapse.

8. When `blast_call` is `Recognised`, `decide` behaves exactly as it does today: same arms,
   same instant, same grounds. Every existing test in `park_decision.rs` must still pass,
   adjusted only where the enum shape forces it.

## Boundaries, at N and at zero

- EMPTY log: `Unrecognised`. Nothing was seen, so nothing is known.
- A log containing TWO recognised phrasings: the WIDEST recognised width wins
  (`Credential` > `Bucket` > `Arm`). A refusal that is both is at least the wider, and
  under-parking is the failure this task exists to stop. Pin it with a log containing both
  `"token limit"` and `"key limit"`.
- A phrasing appearing only inside a longer word (e.g. `keylimit`, no space) does NOT match.
- `known_phrasings()` with its entries iterated must satisfy clause 6 for EVERY entry, not
  for a sampled one.

## Composition of the aggregate return

`blast_call` returns exactly one `BlastCall` for the whole log.
`Unrecognised` is not a width and must never be mapped to one inside this module; only the
caller decides what to do without a width, and `decide` records it as `UnknownBlast` rather
than choosing for it.

## Rules

Pure functions over the arguments given: no file system, no clock, no environment.
Update every call site inside `farmerbob-core` so the crate builds and every existing test
passes; that is part of the task. Every clause above gets a test named for what it pins.
Keep the workspace rustfmt-clean and clippy-clean.
