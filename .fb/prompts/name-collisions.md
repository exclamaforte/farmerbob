<!-- fb:creates crates/farmerbob-core/src/name_collision.rs -->
# Task: two public types with one name, inside one crate, is a collision worth reporting

Rust workspace, already builds. Work only inside `crates/farmerbob-core`.
Create `crates/farmerbob-core/src/name_collision.rs` and declare it in `lib.rs`. Change
nothing else.

## Why this exists

`farmerbob-core` contains FOUR `pub enum Fate`:

```text
finding_fate.rs:53   spec_fate.rs:7   prove_veto.rs:31   verify_plan.rs:32
```

Two of them came from my own specs on the same day. Nothing anywhere compares a spec's
"Exact API" against the names the crate already has, so the collision was invisible until
someone counted. (bead farmerbob-vtp1)

It is not only `Fate`. Within this workspace there are five `Plan`, four `Registry`, four
`Decision`, four `Outcome`, four `Evidence`.

The harm is concrete and this project has paid it repeatedly: today alone, two
`Conformance` enums and two `Plan` enums were found, each a case where a decision already
implemented in `farmerbob-core` was implemented a second time in `fb` because nobody knew
the first existed. In both cases the core version was the more careful one and the harness
ran the other.

This task builds the detector. It does not rename anything.
...
///
/// A public type is a line whose first tokens are `pub enum`, `pub struct`, `pub trait`,
/// `pub type` or `pub union`. This list is a SUPERSET: Rust may gain another item kind, and
/// one not listed here is simply not counted, never guessed at. (bead farmerbob-2ve)
pub fn public_types(source: &str) -> Vec<String>;
```

## Falsifiable clauses

1. A name defined as `pub enum` in two modules is a collision carrying both stems, sorted.

2. The item KINDS mix: `pub struct Foo` in one module and `pub enum Foo` in another is still
   one collision. Two things with one name is the hazard, whatever shape they are.

3. A name defined ONCE is not reported, however many times it is MENTIONED. Pin it with a
   module that uses `Fate` in a function signature and a doc comment but defines nothing.

4. A name defined twice in the SAME module is not a collision between modules; it cannot
   compile, and reporting it here would blame the wrong thing. `modules` is deduplicated, so
   such a name yields a single stem and is not reported.

5. Only `pub` items count. A private `enum Fate` in one module and a `pub enum Fate` in
   another is not a collision: the private one cannot be reached, so nobody can confuse them.

6. A `pub` item inside a `#[cfg(test)]` module does not count. Test scaffolding is not the
   crate's API, and counting it would report every fixture type as a collision.

7. The result is sorted by `name`, and each `Collision::modules` is sorted and deduplicated,
   so two runs over the same input give the same list in the same order.

## Boundaries, at N and at zero

- ZERO modules: an empty vec. Not an error -- a crate with no modules has no collisions.
- ONE module: an empty vec, always. A collision needs two definitions in two modules, and
  clause 4 says a repeat inside one module is not this function's business.
- A module whose source is the EMPTY STRING contributes no names and is not itself an error.
- A name defined in THREE modules is ONE collision carrying three stems, not two collisions
  or three. The unit is the name.

## Composition of the aggregate return

`collisions` returns one entry per colliding NAME, never one per pair and never one per
definition. A name in three modules yields exactly one `Collision` whose `modules` has three
entries. A crate with no collisions yields an empty vec, which is an answer and not an
absence -- this function has nothing it can fail to measure, so it returns a plain `Vec` and
not a `Measurement`.

## Rules

Pure functions over the arguments given: no file system, no clock, no environment. The
caller reads the files. Every clause above gets a test named for what it pins. Keep the
workspace rustfmt-clean and clippy-clean, and keep every existing test passing.
