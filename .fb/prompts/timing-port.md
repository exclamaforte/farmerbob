<!-- fb:creates crates/fb/src/timing_cmd.rs -->
# Task: run_timing is merged and fb-timing.sh still does the arithmetic itself

Rust workspace, already builds. Work only inside `crates/fb`.
Create `crates/fb/src/timing_cmd.rs`. Declare it in `main.rs` with one `mod timing_cmd;` line
and change nothing else. `fb` is a binary crate with no `lib.rs`, so `main.rs` is the only place
that line can go.

## Why this exists

`farmerbob_core::run_timing::derive` turns a run's start time, the files it wrote with their
mtimes, and a line count into three figures: time to first write, the span from first write to
last, and lines per minute. It returns `Measurement<RunTiming>`, `Missing` when the run wrote
nothing, because a run that wrote nothing has no first write and reporting zero would say it
wrote instantly.

It is merged, tested, and reachable from nothing. `fb-timing.sh` is 35 lines of shell doing the
arithmetic itself, and nothing checks the two agree.
...
`Wrote` and `RunTiming` are `farmerbob_core::run_timing`'s. You define neither, and you do not
re-derive any figure: `derive` owns the arithmetic.

## Exit codes, pinned

- `0` — at least one row rendered.
- `1` — no rows. The heading is still printed.

## Falsifiable clauses

1. A row whose `wrote` is non-empty renders one line containing the arm name and the three
   figures `derive` returns for it. Assert the numbers match `run_timing::derive`'s output for
   the same input, rather than recomputing them in the test.
2. A row with an EMPTY `wrote` renders with `ABSENT` in the figure columns, because `derive` is
   `Missing`. The row is still printed: an arm that wrote nothing is a fact worth showing.
3. `lines_per_min` is itself a `Measurement` inside `RunTiming`, so a row can have observed
   ttfw and span and an ABSENT rate. Pin that case — it is the one an implementation that checks
   only the outer `Measurement` gets wrong.
4. Clauses 2 and 3 must be pinned as a PAIR: the whole timing absent, versus present-with-an-
   absent-rate. They render differently and an implementation collapsing them passes each alone.
5. Every row in the input appears in the output, in input order, exactly once. Not sorted, not
   filtered.
6. A heading line names the five columns. Assert it MENTIONS each name case-insensitively; do
   NOT assert layout, padding or separator, none of which this spec fixes.
7. `run` writes exactly what `table` returns and returns 0 for a non-empty input, 1 for empty.
8. `ABSENT` is quoted verbatim by this spec, so it IS pinned and tests may assert it exactly.

## Boundaries, at N and at zero

- ZERO rows: heading only, exit 1.
- A row with exactly ONE written file: `span_s` is whatever `derive` returns for a single write.
  Assert agreement with `derive`; do not assert a value you did not first confirm from it.
- `lines: 0` with files written: the line count renders as `0`, not `ABSENT`. A measured zero is
  an answer. Pin it against clause 2's absent case.
- A `wrote` entry whose mtime is BEFORE `started_s`: whatever `derive` does, this renders.
  Assert agreement, and say in your handoff what that turned out to be.
- Two rows with the same arm name: both render. Not deduplicated.

## Superset status on every enumerated list

The columns are EXACTLY arm, ttfw, span, lines, rate, in that order. The exit codes are EXACTLY
0 and 1.

This module performs NO arithmetic on times or rates. `run_timing::derive` owns all of it. A
subtraction, a division, or a `60` appearing in this file is the defect this task exists to
remove, and it is scored as one.

## Composition of aggregate returns

`table` returns the heading plus one line per row in input order — clause 5 — and clause 2 says
an unmeasurable row is still a row. `run` writes that string and returns a code that says only
whether there were rows, never whether the figures were readable.

## Rules

- No `unwrap()`, `expect()`, `panic!`, `todo!` or `unimplemented!` reachable from input, outside
  `#[cfg(test)]`.
- Add no dependencies. No filesystem: every row arrives as a value, mtimes included.
...
- Derive `Debug, Clone, PartialEq` on `TimingRow`. Not `Eq`: `RunTiming` carries an `f64` rate
  and the crate's own type implements `Eq` manually for that reason. This is pinned because a
  suite asserting with `assert_eq!` fails to COMPILE against a rival that omits `PartialEq`.
