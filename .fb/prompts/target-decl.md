<!-- fb:creates crates/farmerbob-core/src/target_decl.rs -->
# Task: fb-target.sh is sourced by seven scripts and parses the marker with sed

Rust workspace, already builds. Work only inside `crates/farmerbob-core`.
Create `crates/farmerbob-core/src/target_decl.rs`. Declare it in `lib.rs` with one
`pub mod target_decl;` line and change nothing else.

## Why this exists

Every spec declares its deliverable in an HTML comment on the first line:
...
`fb-target.sh` extracts the path and the verb with `sed`, and SEVEN scripts source it —
fb-critique, fb-escalate, fb-speccheck, fb-autopilot, fb-pipeline and more. It is 44 lines of
pure shell and there is no Rust behind it, so the single fact every stage depends on is
recovered by a regex that nothing tests.

`farmerbob_core::precondition` already parses these markers to decide whether a queued spec
still holds against a base, and it is merged and tested. What it does NOT expose is the plain
question every one of those seven scripts asks: *what does this spec declare?*

This task adds that. It does not re-implement the scanner.
...
1. `<!-- fb:creates crates/a/src/b.rs -->` on the first line yields
   `Creates("crates/a/src/b.rs")`.
2. `<!-- fb:modifies crates/a/src/b.rs -->` yields `Modifies(..)` with the same path.
3. The marker is found wherever it appears, not only on line 1. Pin one on a later line.
4. A marker inside a fenced code block is IGNORED. Pin a spec whose only marker is fenced:
   `Absent`. The dispatcher greps the whole prompt, and wave60 refused all four of its own arms
   because an EXAMPLE of a declaration was read as a declaration.
5. A spec with no marker at all: `Absent`.
6. TWO markers outside fences: `Ambiguous`, carrying both paths in appearance order. Pin that
   the vector has two entries and their order — a spec declaring two files has no single
   deliverable, and picking the first silently is how a task gets scored against a file its
   author did not mean.
7. Clauses 1 and 6 must be pinned in tests that differ ONLY in the number of markers. An
   implementation returning the first match passes clause 1 and fails clause 6.
8. `path` returns the carried path for both variants, unchanged — no trimming of directory
   components, no normalisation, no leading-slash handling.
9. `requires_absent` is true for `Creates` and false for `Modifies`. Pin both.
10. An unknown verb — `<!-- fb:deletes foo.rs -->` — is `Absent`, not `Refused`, UNLESS
    `precondition` refuses it. Whatever `farmerbob_core::precondition` does with an unknown
    marker word, this function does the same; it is the existing scanner's rule and you consume
    it. Assert agreement with `precondition` rather than asserting a verb list of your own.

## Boundaries, at N and at zero

- EMPTY spec text: `Absent`.
- A spec that is ONLY a marker, with no other text: the declaration is still found.
- THREE markers: `Ambiguous` with three paths, in order. Not two.
- An unterminated fence — ``` opened and never closed, with a marker after it: the marker is
  inside the fence and is ignored. Pin it; this is the degenerate case.
- A marker with no path, `<!-- fb:creates -->`: NOT pinned here. Whatever `precondition`
  reports, report the same, and say in your handoff which of `Absent` and `Refused` that turned
  out to be.

## Superset status on every enumerated list

`Declaration` has EXACTLY two variants and `NoDeclaration` exactly three. Closed sets.

**You do not write a scanner.** `farmerbob_core::precondition::declarations` already scans a
prompt for `fb:` markers, tracks fenced blocks, and reports refusals with reasons. This module
is a thin question asked of it: take its output and answer "what does this spec declare". A
second scanner in this file is the defect this task exists to avoid, and it is scored as one.

If `precondition`'s output genuinely cannot answer a clause above, say so in your handoff, name
the clause, and extend `precondition`'s caller here rather than duplicating its parsing. **Where
this spec's Exact API and an existing type conflict, the EXACT API WINS** — write the pinned
signature and name the conflict. (bead farmerbob-udp5)

## Composition of aggregate returns

`Ambiguous` carries EVERY declared path in appearance order, not deduplicated: a spec declaring
the same path twice carries it twice, and that is still ambiguous. Pin it.

`declared` returns exactly one `Declaration` or exactly one `NoDeclaration`; there is no partial
answer, and no variant carries both a path and a fault.

## Rules

- No `unwrap()`, `expect()`, `panic!`, `todo!` or `unimplemented!` reachable from input, outside
  `#[cfg(test)]`.
- Add no dependencies, and no `regex`. No filesystem: the spec text arrives as `&str`.
