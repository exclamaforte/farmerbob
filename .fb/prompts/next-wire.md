...
# Task: four subcommands' worth of merged code and one of them is what the orchestrator asked for
...
`crates/fb/src/next_cmd.rs` is merged and reachable from nothing. It gathers the harness's state
from disk, hands it to `farmerbob_core::board::observe` and `attention::rank`, and renders what
to look at first — the mechanism that replaces reading `fb-status.sh` prose and guessing.

`main.rs` declares `mod next_cmd;` and never mentions it again. Until the subcommand exists the
orchestrator still greps.

This is the wiring task, separate by necessity: the task that wrote `next_cmd` was told not to
touch the argument parser, because that would have been a second changed file and a scope
violation.
...
Its arm builds `next_cmd::Paths` from the harness's own locations — the repository root this
binary already resolves for every other subcommand, and the logs directory it already uses —
counts live agents the way `fb status` already does, calls `next_cmd::run` writing to
`std::io::stdout()`, and exits with the returned code via `std::process::exit`.

You add nothing to `next_cmd.rs`. This task changes ONE file.

## Falsifiable clauses

1. `fb next` prints the ranked items and exits 0 when there is something to act on.
2. `fb next` exits 1 when there is nothing to do, and still prints a readable line. It does not
   print nothing.
3. `fb next --limit 1` prints at most one item. `--limit 0` prints all. Pin both.
4. The exit code is `next_cmd::run`'s return value unchanged: 0, 1 and 4 reach the shell as
   themselves. Pin at least that 1 and 4 differ — an idle harness and an unreadable one need
   opposite responses.
5. The live-agent count is obtained the same way `fb status` obtains it, and a count that cannot
   be read is passed through as `Missing`, NOT as 0. Assert that the code path does not
   substitute a zero; if the existing helper is private and cannot be called from a test, say so
   in your handoff and assert what you can.
...
7. `fb next --limit` with a non-numeric value is clap's own usage error, exit 2. Do not catch it.

## Boundaries, at N and at zero

- `--limit 0`: all items. This is the default and clause 3 pins it.
- `--limit` larger than the number of items: all of them, no padding, no error.
- A repository with no `.fb/` directory at all: exit 4, per `next_cmd::run`.
- No `--limit` flag at all: same as `--limit 0`.

## Superset status on every enumerated list

The flags are EXACTLY `--limit`. The exit codes are EXACTLY 0, 1, 2 and 4, where 2 is clap's and
the rest are `next_cmd::run`'s.

This task adds NO logic. Gathering, ranking and rendering all belong to `next_cmd` and the core
modules beneath it. A `match` on an item, a sort, or a second way of counting live agents in
`main.rs` is the defect this task exists to avoid.

## Composition of aggregate returns

The subcommand writes what `next_cmd::run` writes and returns what it returns. Clause 4 is the
statement a shell caller depends on, and clause 2 is the one that keeps "nothing to do" from
looking like "broke silently".

## Testing this

`main.rs` already carries tests for its argument surface; follow that pattern rather than
introducing a new one. If it cannot express a clause — clap exits the process rather than
returning, so clause 7 is the likely one — say so in your handoff, name the clause, and assert
what CAN be asserted, such as that the argument is declared with the type that rejects it.
...
- No `unwrap()`, `expect()`, `panic!`, `todo!` or `unimplemented!` reachable from input, outside
  ...
  its code.
- Add no dependencies. `clap` and `next_cmd` are already present.
