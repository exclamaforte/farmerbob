...
print(tomllib.load(open('$repo/sources.toml','rb'))['source'].get('$arm',{}).get('model',''))")
  cd "$wd" || return 127
  case "$arm" in
    codex-luna)      codex exec --dangerously-bypass-approvals-and-sandbox -m gpt-5.6-luna "$prompt" ;;
    gemini-38-flash) command agy -p "$prompt" --model gemini-3.8-flash-high --add-dir "$wd" \
                         --dangerously-skip-permissions --output-format text ;;
    glm-53-flash)    zcode --prompt "$prompt" ;;
    ifm-*)           set -a; . "$HOME/.config/farmerbob/secrets.env"; set +a
                     opencode run -m "$model" "$prompt" ;;
    or-*)            ori opencode run -m "$model" "$prompt" ;;
